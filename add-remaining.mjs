...
var users = pgTable("users", {
  ...
  // user or admin
  role_type: text("role_type").default("consumer"),
  // Always consumer
  ...
  creator_status: text("creator_status").default("none"),
  // none, pending, approved, verified, suspended
  creator_tier: text("creator_tier").default("new"),
  // new, trusted, verified, partner
  creator_score: integer("creator_score").default(0),
...
var organizationRoles = pgTable("organization_roles", {
  id: serial("id").primaryKey(),
...
var tripCollaborators = pgTable("trip_collaborators", {
  id: serial("id").primaryKey(),
...
var trips = pgTable("trips", {
  ...
  // Trip metadata
  trip_type: text("trip_type").default("personal"),
  ...
  // Percentage
  collaborators: jsonb("collaborators").$type().default([]),
  status: text("status").default("active"),
...
var activities = pgTable("activities", {
  ...
  split_between: integer("split_between").default(1),
...
var groupExpenses = pgTable("group_expenses", {
  id: serial("id").primaryKey(),
  trip_id: integer("trip_id").notNull(),
  activity_id: integer("activity_id"),
  ...
  // User who paid
  split_type: text("split_type").default("equal"),
  // equal, custom, percentage
  split_details: jsonb("split_details").$type().notNull(),
...
var templates = pgTable("templates", {
  ...
  user_id: integer("user_id").notNull(),
  // Creator
  trip_data: jsonb("trip_data").notNull(),
  // Stored itinerary
  destinations: jsonb("destinations").$type().default([]),
...
var templatePurchases = pgTable("template_purchases", {
  ...
  payout_status: text("payout_status").default("pending"),
...
var templateReviews = pgTable("template_reviews", {
  id: serial("id").primaryKey(),
  template_id: integer("template_id").notNull(),
  user_id: integer("user_id").notNull(),
...
var creatorProfiles = pgTable("creator_profiles", {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull().unique(),
  bio: text("bio"),
  website: text("website"),
  social_links: jsonb("social_links").$type().default({}),
...
var creatorBalances = pgTable("creator_balances", {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull().unique(),
...
var viatorCommissions = pgTable("viator_commissions", {
  id: serial("id").primaryKey(),
...
var templateCollections = pgTable("template_collections", {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull(),
  name: text("name").notNull(),
  description: text("description"),
  template_ids: jsonb("template_ids").$type().default([]),
  is_public: boolean("is_public").default(false),
  created_at: timestamp("created_at").defaultNow(),
  updated_at: timestamp("updated_at").defaultNow()
});
var destinations = pgTable("destinations", {
  ...
  top_attractions: jsonb("top_attractions").$type().default([]),
  local_tips: jsonb("local_tips").$type().default([]),
  getting_around: text("getting_around"),
  where_to_stay: text("where_to_stay"),
  food_and_drink: text("food_and_drink"),
  faqs: jsonb("faqs").$type().default([]),
...
var spendControls = pgTable("spend_controls", {
  id: serial("id").primaryKey(),
  organization_id: integer("organization_id").notNull(),
  created_at: timestamp("created_at").defaultNow()
});
var adminBranding = pgTable("admin_branding", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
...
var approvalWorkflows = pgTable("approval_workflows", {
  id: serial("id").primaryKey(),
  organization_id: integer("organization_id").notNull(),
  name: text("name").notNull(),
  created_at: timestamp("created_at").defaultNow()
...
import { pgTable as pgTable2, text as text2, serial as serial2, integer as integer2, boolean as boolean2, timestamp as timestamp2, jsonb as jsonb2 } from "drizzle-orm/pg-core";
var superadminAuditLogs = pgTable2("superadmin_audit_logs", {
  id: serial2("id").primaryKey(),
  admin_user_id: integer2("admin_user_id").references(() => users.id).notNull(),
  action: text2("action").notNull(),
  // CREATE_ORG, UPDATE_USER, RESET_PASSWORD, etc.
  entity_type: text2("entity_type").notNull(),
  // organization, user, system
  entity_id: integer2("entity_id"),
  target_user_id: integer2("target_user_id").references(() => users.id),
  target_organization_id: integer2("target_organization_id").references(() => organizations.id),
  details: jsonb2("details").$type(),
  ip_address: text2("ip_address"),
  user_agent: text2("user_agent"),
  severity: text2("severity").default("info").notNull(),
  // info, warning, critical
  created_at: timestamp2("created_at").defaultNow().notNull()
});
var activeSessions = pgTable2("active_sessions", {
  id: text2("id").primaryKey(),
  // session ID
  user_id: integer2("user_id").references(() => users.id).notNull(),
  username: text2("username").notNull(),
  ip_address: text2("ip_address"),
  user_agent: text2("user_agent"),
  location: text2("location"),
  device_info: jsonb2("device_info").$type(),
  last_activity: timestamp2("last_activity").defaultNow().notNull(),
  expires_at: timestamp2("expires_at"),
  created_at: timestamp2("created_at").defaultNow().notNull()
});
var aiUsageLogs = pgTable2("ai_usage_logs", {
  id: serial2("id").primaryKey(),
  user_id: integer2("user_id").references(() => users.id),
  organization_id: integer2("organization_id").references(() => organizations.id),
  endpoint: text2("endpoint").notNull(),
  prompt_tokens: integer2("prompt_tokens").default(0).notNull(),
  completion_tokens: integer2("completion_tokens").default(0).notNull(),
  total_tokens: integer2("total_tokens").default(0).notNull(),
  cost_cents: integer2("cost_cents").notNull(),
  model: text2("model").notNull(),
  success: boolean2("success").default(true).notNull(),
  error_message: text2("error_message"),
  created_at: timestamp2("created_at").defaultNow().notNull()
});
var superadminFeatureFlags = pgTable2("superadmin_feature_flags", {
  id: serial2("id").primaryKey(),
  flag_name: text2("flag_name").notNull().unique(),
  description: text2("description"),
  default_value: boolean2("default_value").default(false).notNull(),
  is_enabled: boolean2("is_enabled").default(false).notNull(),
  // For backwards compatibility
  created_at: timestamp2("created_at").defaultNow().notNull(),
  updated_at: timestamp2("updated_at").defaultNow().notNull()
});
var organizationFeatureFlags = pgTable2("organization_feature_flags", {
  id: serial2("id").primaryKey(),
  organization_id: integer2("organization_id").references(() => organizations.id).notNull(),
  flag_name: text2("flag_name").notNull(),
  enabled: boolean2("enabled").notNull(),
  created_at: timestamp2("created_at").defaultNow().notNull(),
  updated_at: timestamp2("updated_at").defaultNow().notNull()
});
var superadminBackgroundJobs = pgTable2("superadmin_background_jobs", {
  id: serial2("id").primaryKey(),
  job_type: text2("job_type").notNull(),
  // export_data, send_email, cleanup_logs, etc.
  status: text2("status").default("pending").notNull(),
  // pending, running, completed, failed
  data: jsonb2("data").$type(),
  result: jsonb2("result").$type(),
  error_message: text2("error_message"),
  progress: integer2("progress").default(0).notNull(),
  total: integer2("total").default(100).notNull(),
  started_at: timestamp2("started_at"),
  completed_at: timestamp2("completed_at"),
  created_by: integer2("created_by").references(() => users.id),
  created_at: timestamp2("created_at").defaultNow().notNull()
});
var billingEvents = pgTable2("billing_events", {
  id: serial2("id").primaryKey(),
  organization_id: integer2("organization_id").references(() => organizations.id).notNull(),
  event_type: text2("event_type").notNull(),
  // subscription_created, payment_succeeded, etc.
  stripe_event_id: text2("stripe_event_id"),
  amount_cents: integer2("amount_cents"),
  currency: text2("currency").default("usd"),
  metadata: jsonb2("metadata").$type(),
  created_at: timestamp2("created_at").defaultNow().notNull()
});
var systemActivitySummary = pgTable2("system_activity_summary", {
  id: serial2("id").primaryKey(),
  date: timestamp2("date").notNull(),
  total_users: integer2("total_users").default(0).notNull(),
  active_users: integer2("active_users").default(0).notNull(),
  total_organizations: integer2("total_organizations").default(0).notNull(),
  active_organizations: integer2("active_organizations").default(0).notNull(),
  total_revenue_cents: integer2("total_revenue_cents").default(0).notNull(),
  new_signups: integer2("new_signups").default(0).notNull(),
  created_at: timestamp2("created_at").defaultNow().notNull()
});
...
var db = drizzle(pool, { schema: { ...schema_exports, ...superadmin_schema_exports } });

// add-remaining-activities.js
import { eq, like, and, or } from "drizzle-orm";
var remainingTemplateActivities = {
  ...
  "Mile High on a Low Budget": {
    ...
    activities: [
      ...
      // Day 2
      { day: 2, title: "City Park", time: "9:00 AM", locationName: "City Park", latitude: 39.7476, longitude: -104.949, price: 0, costNotes: "FREE", notes: "Largest park in Denver" },
      ...
      { day: 3, title: "Denver Botanic Gardens", time: "11:00 AM", locationName: "1007 York St", latitude: 39.7322, longitude: -104.961, price: 12, costNotes: "$12 residents", notes: "Discounted for Denver residents" },
  ...
  "Keep Austin Weird on a Budget": {
    ...
    activities: [
      ...
      { day: 1, title: "Food Truck Lunch", time: "12:00 PM", locationName: "South First Food Court", latitude: 30.2504, longitude: -97.755, price: 8, costNotes: "$8 tacos", notes: "Best tacos in Austin" },
      { day: 1, title: "Zilker Park", time: "2:00 PM", locationName: "Zilker Park", latitude: 30.267, longitude: -97.7676, price: 0, costNotes: "FREE", notes: "350 acres of green space" },
      { day: 1, title: "Barton Springs Pool", time: "4:00 PM", locationName: "2201 Barton Springs Rd", latitude: 30.2641, longitude: -97.7706, price: 5, costNotes: "$5 residents", notes: "Natural spring pool, 70\xB0F year-round" },
      // Day 2
      ...
      { day: 2, title: "6th Street Live Music", time: "7:00 PM", locationName: "6th Street", latitude: 30.2672, longitude: -97.739, price: 0, costNotes: "FREE shows", notes: "No cover at many venues" },
      { day: 2, title: "Rainey Street Happy Hour", time: "5:00 PM", locationName: "Rainey Street", latitude: 30.2592, longitude: -97.739, price: 10, costNotes: "$10 drinks/apps", notes: "Historic houses turned bars" },
  ...
  "NOLA Jazz & Soul for Less": {
    ...
    activities: [
      ...
      { day: 1, title: "French Market", time: "12:00 PM", locationName: "French Market", latitude: 29.961, longitude: -90.0607, price: 8, costNotes: "$8 po-boy", notes: "Cheap local food" },
      { day: 1, title: "Royal Street Music", time: "2:00 PM", locationName: "Royal Street", latitude: 29.9574, longitude: -90.065, price: 0, costNotes: "FREE street performers", notes: "Best buskers in the city" },
      ...
      { day: 2, title: "Lafayette Cemetery", time: "11:00 AM", locationName: "1400 Washington Ave", latitude: 29.9285, longitude: -90.085, price: 0, costNotes: "FREE", notes: "Above-ground tombs" },
      ...
      // Day 3
      { day: 3, title: "City Park", time: "9:00 AM", locationName: "City Park", latitude: 29.9934, longitude: -90.098, price: 0, costNotes: "FREE", notes: "1300 acres, larger than Central Park" },
      { day: 3, title: "Beignets at Morning Call", time: "11:00 AM", locationName: "City Park", latitude: 29.9934, longitude: -90.09, price: 4, costNotes: "$4 beignets", notes: "Cheaper than Cafe du Monde" },
  ...
  "Seattle Sleepless & Cheap": {
    ...
    activities: [
      ...
      { day: 1, title: "Original Starbucks", time: "9:30 AM", locationName: "1912 Pike Pl", latitude: 47.61, longitude: -122.3426, price: 4, costNotes: "$4 coffee", notes: "Historic first store" },
...
async function addRemainingActivities() {
  ...
  for (const [templateTitle, data] of Object.entries(remainingTemplateActivities)) {
    const [template] = await db.select().from(templates).where(like(templates.title, `%${templateTitle}%`)).limit(1);
    if (!template) {
    ...
    if (!currentTripData.budget) {
      const totalBudget = data.duration * 60;
