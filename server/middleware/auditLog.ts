 ...
 */
export async function auditLogMiddleware(req: AuditRequest, res: Response, next: NextFunction): Promise<Response | void> {
    // Only log if user is authenticated and org is known
    if (!req.user?.id || !req.user?.organizationId) {
        return next();
    }
    
    // At this point, we know user exists and has required properties
