...
import { z } from 'zod';
// Create a simple UnauthorizedError class since we can't import it
...
export const requireOrganizationContext = (req: Request, res: Response, next: NextFunction) => {
    try {
        const { organizationId } = req.params;
        const orgIdFromBody = req.body?.organizationId;
        const orgIdFromHeader = req.headers['x-organization-id'] as string | undefined;
...
// Using the Request type from custom-request.d.ts which extends Express.Request
