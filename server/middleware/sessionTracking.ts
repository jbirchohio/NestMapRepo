...
export async function trackUserActivity(req: Request, _res: Response, next: NextFunction) {
    if (req.user && req.user.id) {
        const sessionKey = `${req.user.id}_${req.ip}`;
        // Update in-memory session tracking
        activeSessions.set(sessionKey, {
            userId: parseInt(req.user.id, 10),
            organizationId: req.user.organizationId ? parseInt(req.user.organizationId, 10) : 0,
