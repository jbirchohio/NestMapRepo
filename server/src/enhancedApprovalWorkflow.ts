...
import { eq, and, inArray } from './utils/drizzle-shim';
...
export class EnhancedApprovalWorkflow {
  ...
  async processApproval(
    ...
    if (action === 'rejected') {
      ...
      try {
        // Use available notification method or fallback
        console.log('Approval request rejected notification', {
      ...
      if (levelComplete) {
        ...
        if (nextLevel) {
          ...
          try {
            console.log('Approval request approved notification', {
              userId: request.requesterId,
      ...
      try {
        console.log('Approval request delegated notification', {
  ...
  async addComment(requestId: string, userId: number, comment: string, isInternal: boolean = false): Promise<void> {
    ...
    for (const notifyUserId of notifyUserIds.filter(id => id !== userId)) {
      try {
        console.log('Approval comment notification', {
          userId: notifyUserId,
  ...
  private async notifyApprovers(request: ApprovalRequest, level: number): Promise<void> {
    ...
    for (const approver of approverUsers) {
      try {
        ...
        console.log(`Notification: Approval required for ${request.title} - sent to user ${approver.id}`);
      } catch (error) {
