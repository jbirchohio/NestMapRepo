...
// GET /api/analytics/personal - Personal analytics (accessible to all authenticated users)
router.get('/personal', async (req: Request, res: Response) => {
  try {
    ...
    const personalAnalytics = {
      ...
      recentActivity: [
        { date: '2024-01-15', activity: 'Booked flight to New York', type: 'booking' },
        { date: '2024-01-10', activity: 'Completed trip to Boston', type: 'trip' },
        { date: '2024-01-05', activity: 'Created new trip plan', type: 'planning' },
      ],
      userEngagement: {
        tripsThisMonth: 2,
        bookingsThisMonth: 4,
    ...
    const response: ApiResponse<typeof personalAnalytics> = {
      success: true,
    ...
    const response: ApiResponse = {
      success: false,
      error: { message: 'Failed to fetch personal analytics' },
    };
    res.status(500).json(response);
  }
});
...
router.get('/export/csv', requireRole(['admin', 'manager', 'superadmin_owner', 'superadmin_staff']), async (req: Request, res: Response) => {
  try {
    // Generate CSV data
    const csvData = [
      'Date,Trips,Bookings,Revenue',
      '2024-01-01,15,25,5650.00',
      '2024-01-02,12,20,4320.50',
      '2024-01-03,18,30,7890.25',
      '2024-01-04,10,15,3450.75',
      '2024-01-05,22,35,8960.00',
    ].join('\n');
