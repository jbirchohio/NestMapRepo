...
import type { BaseRepositoryImpl } from '../../common/repositories/base.repository.js';
import { logger } from '../../../utils/logger.js';
import type { UserBookingPreferences } from '../../common/interfaces/booking.interfaces.js';
/**
...
export class UserRepositoryImpl extends BaseRepositoryImpl<User, string, Omit<User, 'id' | 'createdAt' | 'updatedAt'>, Partial<Omit<User, 'id' | 'createdAt' | 'updatedAt'>>> implements CommonUserRepository {
    ...
    async findByResetToken(token: string): Promise<User | null> {
        const [user] = await db
            .select()
            .from(users)
            .where(and(eq(users.resetToken, token), gte(users.resetTokenExpires, new Date())))
            .limit(1);
        return user || null;
    }
