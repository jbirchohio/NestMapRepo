...
export const organizations = pgTable('organizations', {
    ...
    stripePayoutStatus: text('stripe_payout_status').$type(),
...
export const trips = pgTable('trips', {
    ...
    collaborators: jsonb('collaborators').$type().default(sql `'[]'::jsonb`), // e.g., [{ userId: 'uuid', role: 'editor' }]
...
export const organizationRoles = pgTable('organization_roles', {
    ...
    permissions: jsonb('permissions').$type().notNull().default(sql `'[]'::jsonb`), // List of permission keys
...
export const organizationMembers = pgTable('organization_members', {
    ...
    permissionsOverride: jsonb('permissions_override').$type().default(sql `'[]'::jsonb`), // Specific permissions that override the role
    invitedBy: uuid('invited_by').references(() => users.id, { onDelete: 'set null' }),
...
export const customDomains = pgTable("custom_domains", {
    ...
    dnsRecords: jsonb("dns_records").$type(), // Required DNS records for setup
...
export const spendPolicies = pgTable("spend_policies", {
    ...
    targetDepartments: jsonb("target_departments").$type(),
    targetUsers: jsonb("target_users").$type(),
    targetRoles: jsonb("target_roles").$type(),
    dailyLimit: integer("daily_limit"),
    weeklyLimit: integer("weekly_limit"),
    monthlyLimit: integer("monthly_limit"),
    annualLimit: integer("annual_limit"),
    categoryLimits: jsonb("category_limits").$type(),
    merchantRestrictions: jsonb("merchant_restrictions").$type(),
    ...
    allowedDays: jsonb("allowed_days").$type(),
    allowedHours: jsonb("allowed_hours").$type(),
    allowedCountries: jsonb("allowed_countries").$type(),
    blockedCountries: jsonb("blocked_countries").$type(),
...
export const reimbursements = pgTable("reimbursements", {
    ...
    expenseIds: jsonb("expense_ids").$type(),
...
export const bookings = pgTable("bookings", {
    ...
    bookingData: jsonb("booking_data").$type().default(sql `'{}'::jsonb`),
    totalAmount: integer("total_amount"), // in cents
    currency: text("currency").default('USD'),
    passengerDetails: jsonb("passenger_details").$type().default(sql `'{}'::jsonb`),
...
export const featureFlags = pgTable("feature_flags", {
    ...
    conditions: jsonb("conditions").$type().default(sql `'{}'::jsonb`),
...
export const tripComments = pgTable("trip_comments", {
    ...
    parentId: uuid("parent_id").references(() => tripComments.id, { onDelete: 'cascade' }), // For threaded comments
    content: text("content").notNull(),
    mentionedUserIds: uuid("mentioned_user_ids").array(), // Store as array of UUIDs
    reactions: jsonb("reactions").$type(), // e.g., { '👍': 10, '❤️': 5 }
...
export const approvalRules = pgTable("approval_rules", {
    ...
    approverRoles: jsonb("approver_roles").$type().default(sql `'["manager"]'::jsonb`),
...
export const approvalRequests = pgTable("approval_requests", {
    ...
    proposedData: jsonb("proposed_data").$type().default(sql `'{}'::jsonb`),
    reason: text("reason"),
