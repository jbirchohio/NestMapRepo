...
import { StripeIssuingService, CardIssuanceRequest, CardControlsUpdate } from '@shared/services/stripeIssuingService';
...
describe('Stripe Issuing Service', () => {
  ...
  test('should issue a new virtual card', async () => {
    ...
    expect(mockStripeInstance.issuing.cardholders.create).toHaveBeenCalledWith(
    ...
    expect(mockStripeInstance.issuing.cards.create).toHaveBeenCalledWith(
      expect.objectContaining({
        cardholder: mockCardholderId,
        currency: 'usd',
        type: 'virtual',
  ...
  test('should throw error when Stripe is not configured', async () => {
    ...
    await expect(issuingService.issueCard(cardRequest)).rejects.toThrow(
      'Stripe not configured. Please provide STRIPE_SECRET_KEY environment variable.'
    );
  });
  ...
  test('should throw error when organization is not Stripe Connect enabled', async () => {
    ...
    await expect(issuingService.issueCard(cardRequest)).rejects.toThrow(
      'Organization is not enabled for Stripe issuing'
    );
  });
});
