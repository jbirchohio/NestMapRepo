...
export async function getCurrentWeather(location: string): Promise<WeatherData | null> {
    try {
        ...
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(location)}&appid=${apiKey}&units=${units}`);
        if (!response.ok) {
...
export async function getWeatherForecast(location: string, dates: string[]): Promise<WeatherData[]> {
    try {
        ...
        const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(location)}&appid=${apiKey}&units=${units}`);
        if (!response.ok) {
