...
export class StripeIssuingService {
    ...
    async updateCardControls(request: CardControlsUpdate) {
        try {
            ...
            const updatedCard = await stripe.issuing.cards.update(cardRecord.stripe_card_id, updateData);
    ...
    private calculateRiskScore(authorization: any /** FIXANYERROR: Replace 'any' */ /** FIXANYERROR: Replace 'any' */ /** FIXANYERROR: Replace 'any' */ /** FIXANYERROR: Replace 'any' */): number {
        ...
        if (authorization.merchant_data?.category === 'gambling')
            score += 50;
        if (authorization.merchant_data?.country !== 'US')
            score += 20;
