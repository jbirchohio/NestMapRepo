...
export const users = pgTable('users', {
    ...
    userActiveIdx: index('users_active_composite_idx').on(table.isActive, table.lockedUntil, table.emailVerified),
...
export const organizations = pgTable('organizations', {
    ...
    // Subscription status
    subscriptionStatus: text('subscription_status').$type<'active' | 'trialing' | 'past_due' | 'canceled' | 'unpaid' | 'incomplete' | 'incomplete_expired' | null>(),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
});
...
export const refreshTokens = pgTable('refresh_tokens', {
    ...
    tokenValidationIdx: index('refresh_tokens_validation_idx').on(table.token, table.revoked, table.expiresAt),
    // Composite index for user token management
    userTokensIdx: index('refresh_tokens_user_tokens_idx').on(table.userId, table.revoked, table.expiresAt),
    // Index for cleanup of expired tokens
    cleanupIdx: index('refresh_tokens_cleanup_idx').on(table.expiresAt, table.revoked)
}));
...
export const trips = pgTable('trips', {
    ...
    collaborators: jsonb('collaborators').$type<Array<{
        userId: string;
        role: string;
    }>>().default(sql `'[]'::jsonb`), // e.g., [{ userId: 'uuid', role: 'editor' }]
...
// Base schemas with explicit types
export const baseUserSchema = z.object({
  email: z.string().email('Invalid email format').min(1, 'Email is required'),
  passwordHash: z.string().min(8, 'Password must be at least 8 characters'),
  // Add other required fields with proper validation
  username: z.string().min(1, 'Username is required'),
  role: z.enum(['super_admin', 'admin', 'manager', 'member', 'guest']).default('member'),
...
// Create schemas for all tables using the createSchema helper

// Create schemas for all tables
const userSchema = createSchema(users);
const organizationSchema = createSchema(organizations);
const tripSchema = createSchema(trips);
const activitySchema = createSchema(activities);
const todoSchema = createSchema(todos);
const noteSchema = createSchema(notes);
const tripCollaboratorSchema = createSchema(tripCollaborators);
const organizationRoleSchema = createSchema(organizationRoles);
const organizationMemberSchema = createSchema(organizationMembers);
const invitationSchema = createSchema(invitations);
const cardTransactionSchema = createSchema(cardTransactions);
const corporateCardSchema = createSchema(corporateCards);
const cardholderSchema = createSchema(cardholders);
const expenseSchema = createSchema(expenses);
const budgetSchema = createSchema(budgets);
const customDomainSchema = createSchema(customDomains);
const whiteLabelSettingSchema = createSchema(whiteLabelSettings);
const whiteLabelRequestSchema = createSchema(whiteLabelRequests);
const tripCommentSchema = createSchema(tripComments);
const adminAuditLogSchema = createSchema(adminAuditLog);

// Export schema types
export type UserSchema = z.infer<typeof userSchema.select>;
export type OrganizationSchema = z.infer<typeof organizationSchema.select>;
export type TripSchema = z.infer<typeof tripSchema.select>;
export type ActivitySchema = z.infer<typeof activitySchema.select>;
export type TodoSchema = z.infer<typeof todoSchema.select>;
export type NoteSchema = z.infer<typeof noteSchema.select>;
export type TripCollaboratorSchema = z.infer<typeof tripCollaboratorSchema.select>;
export type OrganizationRoleSchema = z.infer<typeof organizationRoleSchema.select>;
export type OrganizationMemberSchema = z.infer<typeof organizationMemberSchema.select>;
export type InvitationSchema = z.infer<typeof invitationSchema.select>;
export type CardTransactionSchema = z.infer<typeof cardTransactionSchema.select>;
export type CorporateCardSchema = z.infer<typeof corporateCardSchema.select>;
export type CardholderSchema = z.infer<typeof cardholderSchema.select>;
export type ExpenseSchema = z.infer<typeof expenseSchema.select>;
export type BudgetSchema = z.infer<typeof budgetSchema.select>;
export type CustomDomainSchema = z.infer<typeof customDomainSchema.select>;
export type WhiteLabelSettingSchema = z.infer<typeof whiteLabelSettingSchema.select>;
export type WhiteLabelRequestSchema = z.infer<typeof whiteLabelRequestSchema.select>;
export type TripCommentSchema = z.infer<typeof tripCommentSchema.select>;
export type AdminAuditLogSchema = z.infer<typeof adminAuditLogSchema.select>;

// Export schema instances for backward compatibility
export const {
...
export const {
  insert: insertActivitySchema,
  select: selectActivitySchema
} = activitySchema;

export const {
  insert: insertTodoSchema,
  select: selectTodoSchema
} = todoSchema;

export const {
  insert: insertNoteSchema,
  select: selectNoteSchema
} = noteSchema;
...
export const {
  insert: insertCardholderSchema,
  select: selectCardholderSchema
} = cardholderSchema;
...
export const {
  insert: insertCustomDomainSchema,
  select: selectCustomDomainSchema
} = customDomainSchema;

export const {
  insert: insertWhiteLabelSettingSchema,
  select: selectWhiteLabelSettingSchema
} = whiteLabelSettingSchema;

export const {
  insert: insertWhiteLabelRequestSchema,
  select: selectWhiteLabelRequestSchema
