...
export const insertProposalSchema = createInsertSchema(proposals, {
  ...
  status: (schema) => schema.status.oneOf([...proposalStatusEnum]).default('draft'),
  clientName: (schema) => schema.clientName.min(1).max(255),
  clientEmail: (schema) => schema.clientEmail.email(),
  clientCompany: (schema) => schema.clientCompany.optional(),
  subtotal: (schema) => schema.subtotal.min(0).optional(),
  taxAmount: (schema) => schema.taxAmount.min(0).optional(),
  discountAmount: (schema) => schema.discountAmount.min(0).optional(),
  totalAmount: (schema) => schema.totalAmount.min(0).optional(),
  currency: (schema) => schema.currency.length(3).optional(),
  termsAndConditions: (schema) => schema.termsAndConditions.optional(),
