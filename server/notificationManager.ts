...
export async function sendNotification(notification: NotificationData, userEmail: string, preferences: UserNotificationPreferences, pushSubscriptions: PushSubscription[] = []): Promise<{
    emailSent: boolean;
    pushSent: number;
    pushFailed: number;
}> {
...
function shouldSendEmailForType(type: string, preferences: UserNotificationPreferences): boolean {
    switch (type) {
        case 'trip_shared':
        case 'team_invite':
...
export async function notifyTripShared(userId: number, tripTitle: string, sharedByName: string, tripId: number): Promise<void> {
...
export async function notifyBookingConfirmed(userId: number, bookingType: string, bookingDetails: string, bookingId: string): Promise<void> {
    const userEmail = getUserEmail(userId);
    const preferences = getUserPreferences(userId);
    const pushSubscriptions = getUserPushSubscriptions(userId);
    if (!userEmail)
...
export async function notifyActivityReminder(userId: number, activityTitle: string, timeUntil: string, tripId: number): Promise<void> {
    const userEmail = getUserEmail(userId);
    const preferences = getUserPreferences(userId);
    const pushSubscriptions = getUserPushSubscriptions(userId);
    if (!userEmail)
