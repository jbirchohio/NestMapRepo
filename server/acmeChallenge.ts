import type { Request as ExpressRequest, Response } from 'express.js';
import { db } from './db-connection.js.js';
import { customDomains } from './db/schema.js.js';
 ...
 */
export function storeACMEChallenge(
  domain: string,
  token: string,
  keyAuthorization: string,
 ...
 */
export async function validateACMEChallenge(
  domain: string,
  token: string,
  expectedKeyAuthorization: string
): Promise<boolean> {
  try {
...
export function getChallengeStats(): {
  ...
  challengesByDomain: { [domain: string]: number };
} {
  const now = new Date();
