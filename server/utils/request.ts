 ...
 */
export const extendRequest = (req: AuthenticatedRequest, _res: Response, next: NextFunction): void => {
...
export const requirePermission = (permissions: string | string[]) => {
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        ...
        const hasPermission = requiredPermissions.every(permission => req.hasPermission(permission));
        if (!hasPermission) {
            return res.status(403).json({
                success: false,
                error: {
