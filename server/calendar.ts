...
export function generateICalContent(trip: Trip, activities: Activity[]): string {
  const events = activities.map(activity => {
    ...
    return `BEGIN:VEVENT
UID:remvana-${activity.id}-${Date.now()}@remvana.com
DTSTAMP:${formatDateForICal(new Date())}
DTSTART:${formatDateForICal(startDate)}
DTEND:${formatDateForICal(endDate)}
SUMMARY:${activity.title}
DESCRIPTION:${activity.notes || 'Activity from Remvana trip: ' + trip.title}
...
VERSION:2.0
PRODID:-//Remvana//Trip Calendar//EN
CALSCALE:GREGORIAN
METHOD:PUBLISH
X-WR-CALNAME:${trip.title} - Remvana Trip
X-WR-CALDESC:Trip itinerary created with Remvana
${events}
END:VCALENDAR`;
}
