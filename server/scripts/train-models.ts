...
import { trips, activities, expenses } from '../db/schema.js.js';
...
async function trainRecommendationModel() {
  ...
  for (const row of rows) {
    if (!row.tag || !row.userId) continue;
  ...
  await fs.writeFile(
    'data/models/recommendation-model.json',
    JSON.stringify(
      {
        model: nb.toJSON(),
...
async function trainSpendForecastModel() {
  ...
  await fs.mkdir('data/models', { recursive: true });
  await fs.writeFile(
    'data/models/spend-forecast-model.json',
    JSON.stringify({ model: mlr.toJSON() }, null, 2)
  );
