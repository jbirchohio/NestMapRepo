...
import { users, organizations, userRoleEnum } from '../shared/schema.ts';
...
export class SSOManager {
    ...
    async configureSSOForOrganization(organizationId: string, config: Omit<SSOConfig, 'organizationId'>): Promise<void> {
        ...
        await auditLogger.logAdminAction('system', // System user
        organizationId, 'sso_configured', { provider: config.provider, entityId: config.entityId });
    }
    async processSAMLResponse(samlResponse: string, relayState?: string): Promise<{
        user: SSOUser;
        organizationId: string;
    } | null> {
        try {
            ...
            await auditLogger.logAuth('system', // Will be updated after user creation
            organizationId, 'sso_login_success', { provider: 'saml', entityId: parsedResponse.entityId });
    ...
    async processOAuthCallback(code: string, state: string): Promise<{
        ...
        try {
            ...
            await auditLogger.logAuth('system', organizationId, 'sso_login_success', { provider: config.provider, email: user.email });
            return { user, organizationId };
        }
        catch (error) {
    ...
    async createOrUpdateSSOUser(ssoUser: SSOUser, organizationId: string): Promise<any> {
        ...
        const [existingUser] = await db
            .select()
            .from(users)
            .where(and(eq(users.email, ssoUser.email), eq(users.organizationId, organizationId)));
        if (existingUser) {
            ...
            const [updatedUser] = await db
                ...
                .returning();
            await auditLogger.logAuth(updatedUser.id, organizationId, 'sso_user_updated', { email: ssoUser.email, groups: ssoUser.groups });
            return updatedUser;
        }
        else {
            // Create new user
            ...
            const [newUser] = await db
                ...
                .values(userData)
                .returning();
            await auditLogger.logAuth(newUser.id, organizationId, 'sso_user_created', { email: ssoUser.email, role, groups: ssoUser.groups });
            return newUser;
        }
    }
