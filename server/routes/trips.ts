...
router.post("/", enforceTripLimit(), validateAndSanitizeRequest({ body: insertTripSchema }), async (req: Request, res: Response) => {
    try {
        ...
        // Log this activity
        await logUserActivity(req.user.userId, 'create_trip', req.user.organizationId, { tripId: newTrip.id, tripTitle: newTrip.title }, req.ip, req.headers['user-agent']);
...
router.put("/:id", validateAndSanitizeRequest({ params: idParamSchema, body: insertTripSchema.partial() }), async (req: Request, res: Response) => {
    try {
        ...
        // Log this activity
        await logUserActivity(req.user.userId, 'update_trip', req.user.organizationId, { tripId: updatedTrip.id, tripTitle: updatedTrip.title, changes: updateData }, req.ip, req.headers['user-agent']);
...
router.delete("/:id", validateAndSanitizeRequest({ params: idParamSchema }), async (req: Request, res: Response) => {
    try {
        ...
        // Log this activity
        await logUserActivity(req.user.userId, 'delete_trip', req.user.organizationId, { tripId: existingTrip.id, tripTitle: existingTrip.title }, req.ip, req.headers['user-agent']);
...
router.put("/:id/travelers/:travelerId", validateAndSanitizeRequest({ params: tripAndTravelerIdParamsSchema, body: travelerBodySchema.partial() }), async (req: Request, res: Response) => {
    try {
        const { id: tripId, travelerId } = req.params as {
            id: string;
            travelerId: string;
        }; // Validated and coerced by middleware
