...
import { tripComments, activityLog, trips, users, insertTripCommentSchema } from '../shared/src/schema.ts';
import { z } from 'zod';
...
router.get('/trips/:tripId/comments', validateJWT, injectOrganizationContext, validateOrganizationAccess, async (req, res) => {
    try {
        ...
        const comments = await db
            ...
            .where(and(eq(tripComments.trip_id, tripId), eq(tripComments.organization_id, organizationId)))
...
router.patch('/comments/:commentId/resolve', async (req, res) => {
    try {
        ...
        const [comment] = await db
            .select()
            .from(tripComments)
            .where(and(eq(tripComments.id, commentId), eq(tripComments.organization_id, organizationId)));
        if (!comment) {
...
router.get('/trips/:tripId/activity', async (req, res) => {
    try {
        ...
        const activities = await db
            ...
            .where(and(eq(activityLog.tripId, tripId), eq(activityLog.organizationId, organizationId)))
...
export async function logTripActivity(tripId: number, userId: number, organizationId: number, activityData: {
    action: string;
    entityType: string;
    entityId?: number;
