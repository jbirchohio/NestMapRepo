...
router.post('/members/invite', requireOrgPermission('inviteMembers'), async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const [existingMember] = await db
            ...
            .where(and(eq(organizationMembers.user_id, existingUser.id), eq(organizationMembers.organization_id, req.user.organization_id)));
        if (existingMember) {
...
router.patch('/members/:memberId', requireOrgPermission('assignRoles'), async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const [existingMember] = await db
            .select()
            .from(organizationMembers)
            .where(and(eq(organizationMembers.id, memberId), eq(organizationMembers.organization_id, req.user.organization_id)));
        if (!existingMember) {
...
router.get('/roles', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const assignableRoles = roles.filter(role => !req.userOrgRole || canAssignRole(req.userOrgRole, role.value as OrganizationRole));
    res.json({
