...
router.post("/create-account", jwtAuthMiddleware, async (req, res) => {
  try {
    if (!req.user?.organization_id) {
      return res.status(400).json({ error: "No organization found" });
    }
...
router.post("/onboarding-link", jwtAuthMiddleware, async (req, res) => {
  try {
    if (!req.user?.organization_id) {
      return res.status(400).json({ error: "No organization found" });
    }
...
router.get("/account-status", jwtAuthMiddleware, async (req, res) => {
  try {
    if (!req.user?.organization_id) {
      return res.status(400).json({ error: "No organization found" });
    }
...
router.post("/setup-funding", jwtAuthMiddleware, async (req, res) => {
  try {
    if (!req.user?.organization_id) {
      return res.status(400).json({ error: "No organization found" });
    }
...
router.post("/add-funds", jwtAuthMiddleware, async (req, res) => {
  try {
    if (!req.user?.organization_id) {
      return res.status(400).json({ error: "No organization found" });
    }
