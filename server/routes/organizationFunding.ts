...
router.post("/create-account", validateJWT, async (req, res) => {
    try {
        ...
        const account = await organizationFundingService.createConnectedAccount(req.user.organization_id, validatedData);
        res.json({
            success: true,
...
router.post("/onboarding-link", validateJWT, async (req, res) => {
    try {
        ...
        const link = await organizationFundingService.createOnboardingLink(req.user.organization_id, returnUrl, refreshUrl);
        res.json({
            success: true,
...
router.post("/add-funds", validateJWT, async (req, res) => {
    try {
        ...
        const result = await organizationFundingService.addFundsToOrganization(req.user.organization_id, amount);
        res.json({
            success: true,
