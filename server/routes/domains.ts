import type { Express, Request, Response } from '../../express-augmentations.js';
...
export function registerDomainRoutes(app: Express) {
    ...
    app.get<{}, {
        domains: Domain[];
    }>("/api/domains", async (req: AuthenticatedRequest, res: Response<{
        domains: Domain[];
    }>) => {
        if (!req.user) {
    ...
    app.post<{
        ...
        success: boolean;
        message: string;
    }>("/api/domains/:domainId/verify", async (req: AuthenticatedRequest, res: Response<{
        success: boolean;
        message: string;
    }>) => {
        if (!req.user) {
        ...
        try {
            ...
            const [domain] = await db
                .select()
                .from(customDomains)
                .where(and(eq(customDomains.id, parseInt(domainId)), eq(customDomains.organization_id, organizationId!)));
            if (!domain) {
    ...
    app.delete<{
        ...
        success: boolean;
        message: string;
    }>("/api/domains/:domainId", async (req: AuthenticatedRequest, res: Response<{
        success: boolean;
        message: string;
    }>) => {
        if (!req.user) {
    ...
    app.get<{
        domain: string;
    }>("/api/domains/:domain/branding", async (req: Request<{
        domain: string;
    }>, res: Response) => {
        ...
        try {
            const result = await db
                ...
                .where(and(eq(customDomains.domain, domain), eq(customDomains.status, 'active'), eq(whiteLabelSettings.status, 'approved')))
                .limit(1);
    ...
    app.get<{}, any>("/api/domains/dashboard", async (req: AuthenticatedRequest, res: Response) => {
        if (!req.user) {
            return res.status(401).json({ error: "Authentication required" });
        }
        const organizationId = req.user.organization_id;
