...
type AuthenticatedHandler = (req: AuthenticatedRequest, res: Response, next: NextFunction) => Promise<void> | void;
...
const logAdminAction = async (adminUserId: string, actionType: string, targetOrgId?: string, actionData: Record<string, any> = {}, ipAddress?: string): Promise<void> => {
    try {
        await auditLogger.logAdminAction(adminUserId, targetOrgId || '', actionType, actionData, ipAddress);
    }
    catch (error) {
...
router.get('/organizations', validateJWT, requireRole('admin'), async (req: Request, res: Response) => {
    ...
    try {
        ...
        await auditLogger.logAdminAction(user.id, user.organizationId || '', 'organizations_viewed', { count: orgs.length }, req.ip);
...
router.get('/organizations/:orgId', validateJWT, requireRole('admin'), validateAndSanitizeRequest({ params: orgIdParamSchema }), async (req: Request, res: Response) => {
    if (!isAuthenticatedRequest(req)) {
        return res.status(401).json({ error: 'Authentication required' });
    }
    const authReq = req as AuthenticatedRequest;
    ...
    try {
        ...
        await auditLogger.logAdminAction(user.id, user.organizationId || orgId, 'organization_viewed', { organizationId: orgId }, authReq.ip);
        return res.status(200).json({
            success: true,
            data: org
        });
...
router.put('/organizations/:orgId', validateJWT, requireRole('admin'), validateAndSanitizeRequest({
    ...
    try {
        ...
        if (filteredData.name)
            updateData.name = filteredData.name;
        if (filteredData.plan)
            updateData.plan = filteredData.plan;
        if (filteredData.subscriptionStatus)
            updateData.subscriptionStatus = filteredData.subscriptionStatus;
        ...
        try {
            await logAdminAction(user.id, 'organization_updated', orgId, {
...
router.patch('/white-label/requests/:requestId', validateJWT, requireRole('admin'), validateAndSanitizeRequest({
    params: z.object({
        requestId: z.string().uuid('Invalid request ID format')
    }),
    ...
    try {
        ...
        const [updatedRequest] = await db
            ...
            .returning();
        // Log the admin action
        await logAdminAction(req.user.id, 'update_white_label_request', updatedRequest.organizationId, {
...
router.get('/activity-logs', validateJWT, requireRole('admin'), async (req: Request, res: Response) => {
    ...
    const user = authReq.user;
    try {
        const { userId, action, organizationId, startDate, endDate, page = '1', limit = '50' } = req.query as {
            userId?: string;
            action?: string;
            organizationId?: string;
            startDate?: string;
        ...
        const total = await db
            ...
            .from(userActivityLogs)
            .where(and(userId ? eq(userActivityLogs.userId, userId) : undefined, action ? eq(userActivityLogs.action, action) : undefined, startDate ? gte(userActivityLogs.createdAt, new Date(startDate)) : undefined, endDate ? lte(userActivityLogs.createdAt, new Date(endDate)) : undefined))
        ...
        const logs = await db.query.userActivityLogs.findMany({
            where: (log, { and, eq, gte, lte }) => {
                ...
                if (action)
                    conditions.push(eq(log.action, action));
                if (startDate)
                    conditions.push(gte(log.createdAt, new Date(startDate)));
                if (endDate)
                    conditions.push(lte(log.createdAt, new Date(endDate)));
        ...
        // Log the admin action
        await logAdminAction(user.id, 'VIEW_ACTIVITY_LOGS', user.organizationId || user.organization_id, {
