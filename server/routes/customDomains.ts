...
function createHandler(handler: (req: CustomRequest, res: Response, next: NextFunction) => Promise<void> | void): RequestHandler {
...
router.get('/organizations/:orgId/domains', requireOrgPermission('view_domains') as unknown as RequestHandler, createHandler(async (req: CustomRequest, res: Response) => {
...
router.post('/organizations/:orgId/domains', requireOrgPermission('manage_domains') as unknown as RequestHandler, createHandler(async (req: CustomRequest, res: Response) => {
...
router.post('/organizations/:orgId/domains/:domainId/verify', requireOrgPermission('manage_domains') as unknown as RequestHandler, createHandler(async (req: CustomRequest, res: Response) => {
    const orgId = req.params.orgId;
...
router.get('/organizations/:orgId/domains/:domainId', requireOrgPermission('view_domains') as unknown as RequestHandler, createHandler(async (req: CustomRequest, res: Response) => {
    const orgId = req.params.orgId;
    const domainId = req.params.domainId;
    if (!orgId || !domainId) {
        sendJsonResponse(res, 400, { error: 'Invalid organization or domain ID' });
    ...
    try {
        ...
        const [domain] = await db.select().from(customDomains).where(and(eq(customDomains.id, domainId), eq(customDomains.organizationId, orgId)));
        if (!domain) {
            sendJsonResponse(res, 404, { error: 'Domain not found or does not belong to this organization' });
            return;
        }
...
router.delete('/organizations/:orgId/domains/:domainId', requireOrgPermission('manage_domains') as unknown as RequestHandler, createHandler(async (req: CustomRequest, res: Response) => {
    const orgId = req.params.orgId;
    const domainId = req.params.domainId;
    if (!orgId || !domainId) {
        sendJsonResponse(res, 400, { error: 'Invalid organization or domain ID' });
    ...
    try {
        ...
        const [domainToDelete] = await db
            .select()
            .from(customDomains)
            .where(and(eq(customDomains.id, domainId), eq(customDomains.organizationId, orgId)));
        if (!domainToDelete) {
...
router.get('/domains/:domain/branding', createHandler(async (req: CustomRequest, res: Response, _next: NextFunction) => {
    ...
    try {
        const result = await db
            ...
            .from(customDomains)
            .leftJoin(whiteLabelSettings, eq(customDomains.organizationId, whiteLabelSettings.organizationId))
            .where(and(eq(customDomains.domainName, domain), eq(customDomains.status, 'active'), eq(whiteLabelSettings.status, 'approved')))
            .limit(1);
...
router.get('/domains/dashboard', authenticate as unknown as RequestHandler, validateOrganizationAccess as unknown as RequestHandler, (async (req: CustomRequest, res: Response) => {
    if (!req.user) {
