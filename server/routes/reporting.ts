...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const [tripStats, expenseStats, bookingStats, userEngagement, approvalStats, costBreakdown] = analytics;
        res.json({
            period,
            overview: {
...
async function getTripAnalytics(organizationId: number, startDate: Date, endDate: Date) {
    const tripCount = await db
        .select({ count: count() })
        .from(trips)
        .where(and(eq(trips.organization_id, organizationId), gte(trips.created_at, startDate), lte(trips.created_at, endDate)));
...
async function getExpenseAnalytics(organizationId: number, startDate: Date, endDate: Date) {
    const expenseStats = await db
        ...
        .from(expenses)
        .where(and(eq(expenses.organization_id, organizationId), gte(expenses.createdAt, startDate), lte(expenses.createdAt, endDate)));
...
async function getBookingAnalytics(organizationId: number, startDate: Date, endDate: Date) {
    const bookingStats = await db
        ...
        .from(bookings)
        .where(and(eq(bookings.organization_id, organizationId), gte(bookings.createdAt, startDate), lte(bookings.createdAt, endDate)));
    return {
...
async function getApprovalAnalytics(organizationId: number, startDate: Date, endDate: Date) {
    const approvalStats = await db
        ...
        .where(and(eq(approvalRequests.organization_id, organizationId), gte(approvalRequests.createdAt, startDate), lte(approvalRequests.createdAt, endDate)))
...
async function getExpenseCompliance(organizationId: number, startDate: Date, endDate: Date) {
    ...
    const expensesWithReceipts = await db
        .select({ count: count() })
        .from(expenses)
        .where(and(eq(expenses.organization_id, organizationId), gte(expenses.createdAt, startDate), lte(expenses.createdAt, endDate), sql `${expenses.receiptUrl} IS NOT NULL`));
...
async function getApprovalCompliance(organizationId: number, startDate: Date, endDate: Date) {
    ...
    const processedOnTime = await db
        .select({ count: count() })
        .from(approvalRequests)
        .where(and(eq(approvalRequests.organization_id, organizationId), gte(approvalRequests.createdAt, startDate), lte(approvalRequests.createdAt, endDate), sql `${approvalRequests.approvedAt} <= ${approvalRequests.dueDate}`, sql `${approvalRequests.status} = 'approved'`));
...
async function getTaxComplianceData(organizationId: number, startDate: Date, endDate: Date) {
    ...
    const taxDeductibleExpenses = await db
        ...
        .from(expenses)
        .where(and(eq(expenses.organization_id, organizationId), gte(expenses.createdAt, startDate), lte(expenses.createdAt, endDate), sql `${expenses.category} IN ('meals', 'accommodation', 'transportation')`))
        .groupBy(expenses.category);
