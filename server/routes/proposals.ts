...
router.post('/', validateJWT, injectOrganizationContext, validateOrganizationAccess, validateAndSanitizeRequest({ body: insertProposalSchema }), async (req: AuthenticatedRequest, res: Response) => {
    try {
...
router.get('/', validateJWT, injectOrganizationContext, validateOrganizationAccess, async (req: AuthenticatedRequest, res: Response) => {
    try {
...
router.get('/:id', validateJWT, injectOrganizationContext, validateOrganizationAccess, async (req: AuthenticatedRequest<{
    id: string;
}>, res: Response) => {
    try {
        const { id } = req.params;
...
router.patch('/:id/status', validateJWT, injectOrganizationContext, validateOrganizationAccess, validateAndSanitizeRequest({ body: z.object({ status: proposalStatusEnum }) }), async (req: AuthenticatedRequest<{
    id: string;
}>, res: Response) => {
    try {
        const { id } = req.params;
...
router.patch('/:id/sign', validateJWT, validateAndSanitizeRequest({
...
router.patch('/:id/invoice', validateJWT, validateAndSanitizeRequest({ body: z.object({ invoiceId: z.string().uuid() }) }), async (req: AuthenticatedRequest<{
    id: string;
}>, res: Response) => {
    try {
        const { id } = req.params;
