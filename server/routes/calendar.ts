...
import { calendarIntegrations, trips, activities } from '../shared/src/schema.ts';
...
router.post('/generate-ical/:tripId', async (req, res) => {
    try {
        ...
        const [trip] = await db
            .select()
            .from(trips)
            .where(and(eq(trips.id, tripId), eq(trips.organization_id, organizationId)));
        if (!trip) {
        ...
        const tripActivities = await db
            .select()
            .from(activities)
            .where(and(eq(activities.trip_id, tripId), eq(activities.organization_id, organizationId)));
...
router.get('/integrations', async (req, res) => {
    try {
        ...
        const integrations = await db
            ...
            .where(and(eq(calendarIntegrations.user_id, userId), eq(calendarIntegrations.organization_id, organizationId)));
...
function generateICalendar(trip: any, activities: any[]): string {
    ...
    // Main trip event
    ical.push('BEGIN:VEVENT', `UID:trip-${trip.id}@nestmap.com`, `DTSTART;VALUE=DATE:${formatDate(startDate)}`, `DTEND;VALUE=DATE:${formatDate(new Date(endDate.getTime() + 86400000))}`, // Add 1 day
    `SUMMARY:Business Trip: ${trip.title}`, `DESCRIPTION:${trip.description || 'Business travel to ' + trip.city}`, `LOCATION:${trip.city}`, 'STATUS:CONFIRMED', 'BEGIN:VALARM', 'TRIGGER:-P1D', 'ACTION:DISPLAY', 'DESCRIPTION:Business trip starting tomorrow', 'END:VALARM', 'END:VEVENT');
    // Activity events
    activities.forEach((activity, index) => {
        ...
        ical.push('BEGIN:VEVENT', `UID:activity-${activity.id}@nestmap.com`, `DTSTART:${formatDateTime(activityDate)}`, `DTEND:${formatDateTime(new Date(activityDate.getTime() + 3600000))}`, // 1 hour duration
        `SUMMARY:${activity.title}`, `DESCRIPTION:${activity.description || ''}`, `LOCATION:${activity.location || trip.city}`, 'STATUS:CONFIRMED', 'END:VEVENT');
    });
