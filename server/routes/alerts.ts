...
import { adminAuditLog } from "../shared/src/schema.ts";
...
export function createAlert(type: SystemAlert['type'], category: SystemAlert['category'], title: string, message: string, metadata?: Record<string, any>): SystemAlert {
...
export function checkSystemHealthAndGenerateAlerts(metrics: {
    ...
    const now = new Date();
    const recentAlerts = systemAlerts.filter(alert => new Date(alert.timestamp).getTime() > now.getTime() - 300000 // Last 5 minutes
    );
    // Memory alerts
    if (metrics.memory.utilization > 90) {
        const hasRecentMemoryAlert = recentAlerts.some(alert => alert.category === 'system' && alert.title.includes('Memory'));
        if (!hasRecentMemoryAlert) {
            createAlert('critical', 'system', 'Critical Memory Usage', `System memory utilization at ${metrics.memory.utilization.toFixed(1)}%. Immediate action required.`, { memoryUtilization: metrics.memory.utilization });
        }
    }
    else if (metrics.memory.utilization > 75) {
        const hasRecentMemoryAlert = recentAlerts.some(alert => alert.category === 'system' && alert.title.includes('Memory'));
        if (!hasRecentMemoryAlert) {
            createAlert('warning', 'system', 'High Memory Usage', `System memory utilization at ${metrics.memory.utilization.toFixed(1)}%. Consider optimization.`, { memoryUtilization: metrics.memory.utilization });
        }
    }
    // CPU alerts
    if (metrics.cpu.usage > 85) {
        const hasRecentCpuAlert = recentAlerts.some(alert => alert.category === 'performance' && alert.title.includes('CPU'));
        if (!hasRecentCpuAlert) {
            createAlert('critical', 'performance', 'Critical CPU Usage', `CPU usage at ${metrics.cpu.usage.toFixed(1)}%. System performance severely impacted.`, { cpuUsage: metrics.cpu.usage });
        }
    }
    else if (metrics.cpu.usage > 70) {
        const hasRecentCpuAlert = recentAlerts.some(alert => alert.category === 'performance' && alert.title.includes('CPU'));
        if (!hasRecentCpuAlert) {
            createAlert('warning', 'performance', 'High CPU Usage', `CPU usage at ${metrics.cpu.usage.toFixed(1)}%. Monitor system performance.`, { cpuUsage: metrics.cpu.usage });
        }
    }
    // Disk alerts
    if (metrics.disk.utilization > 95) {
        const hasRecentDiskAlert = recentAlerts.some(alert => alert.category === 'system' && alert.title.includes('Disk'));
        if (!hasRecentDiskAlert) {
            createAlert('critical', 'system', 'Critical Disk Space', `Disk utilization at ${metrics.disk.utilization.toFixed(1)}%. Storage cleanup required.`, { diskUtilization: metrics.disk.utilization });
        }
    }
    else if (metrics.disk.utilization > 85) {
        const hasRecentDiskAlert = recentAlerts.some(alert => alert.category === 'system' && alert.title.includes('Disk'));
        if (!hasRecentDiskAlert) {
            createAlert('warning', 'system', 'Low Disk Space', `Disk utilization at ${metrics.disk.utilization.toFixed(1)}%. Plan storage cleanup.`, { diskUtilization: metrics.disk.utilization });
        }
    }
    // Performance alerts
    if (metrics.performance.errorRate > 5) {
        const hasRecentErrorAlert = recentAlerts.some(alert => alert.category === 'performance' && alert.title.includes('Error Rate'));
        if (!hasRecentErrorAlert) {
            createAlert('critical', 'performance', 'High Error Rate', `Error rate at ${metrics.performance.errorRate.toFixed(2)}%. System reliability compromised.`, { errorRate: metrics.performance.errorRate });
        }
    }
    else if (metrics.performance.errorRate > 1) {
        const hasRecentErrorAlert = recentAlerts.some(alert => alert.category === 'performance' && alert.title.includes('Error Rate'));
        if (!hasRecentErrorAlert) {
            createAlert('warning', 'performance', 'Elevated Error Rate', `Error rate at ${metrics.performance.errorRate.toFixed(2)}%. Monitor application stability.`, { errorRate: metrics.performance.errorRate });
        }
    }
    // Response time alerts
    if (metrics.performance.responseTimeP95 > 5000) {
        const hasRecentResponseAlert = recentAlerts.some(alert => alert.category === 'performance' && alert.title.includes('Response Time'));
        if (!hasRecentResponseAlert) {
            createAlert('critical', 'performance', 'Severe Response Time Degradation', `95th percentile response time at ${metrics.performance.responseTimeP95.toFixed(0)}ms. User experience severely impacted.`, { responseTimeP95: metrics.performance.responseTimeP95 });
        }
    }
    else if (metrics.performance.responseTimeP95 > 2000) {
        const hasRecentResponseAlert = recentAlerts.some(alert => alert.category === 'performance' && alert.title.includes('Response Time'));
        if (!hasRecentResponseAlert) {
            createAlert('warning', 'performance', 'Slow Response Times', `95th percentile response time at ${metrics.performance.responseTimeP95.toFixed(0)}ms. Performance optimization needed.`, { responseTimeP95: metrics.performance.responseTimeP95 });
        }
    }
    // Network connection alerts
    if (metrics.network.activeConnections > 1000) {
        const hasRecentNetworkAlert = recentAlerts.some(alert => alert.category === 'network' && alert.title.includes('Connections'));
        if (!hasRecentNetworkAlert) {
            createAlert('warning', 'network', 'High Connection Count', `${metrics.network.activeConnections} active connections. Monitor for potential DDoS or traffic spike.`, { activeConnections: metrics.network.activeConnections });
        }
    }
}
...
export async function generateSecurityAlerts(): Promise<void> {
    try {
        ...
        const recentFailedLogins = await db
            .select()
            .from(adminAuditLog)
            .where(and(eq(adminAuditLog.action_type, 'failed_login'), gte(adminAuditLog.timestamp, fifteenMinutesAgo)))
            .orderBy(desc(adminAuditLog.timestamp));
        if (recentFailedLogins.length >= 5) {
            const uniqueIPs = new Set(recentFailedLogins.map(log => log.ip_address));
            createAlert('warning', 'security', 'Multiple Failed Login Attempts', `${recentFailedLogins.length} failed login attempts from ${uniqueIPs.size} unique IP(s) in the last 15 minutes.`, {
        ...
        const suspiciousActions = await db
            .select()
            .from(adminAuditLog)
            .where(and(gte(adminAuditLog.timestamp, fifteenMinutesAgo)))
            .orderBy(desc(adminAuditLog.timestamp));
        const adminActionCount = suspiciousActions.filter(log => ['user_role_change', 'user_deleted', 'system_config_change'].includes(log.action_type)).length;
        if (adminActionCount >= 3) {
            createAlert('info', 'security', 'High Administrative Activity', `${adminActionCount} administrative actions performed in the last 15 minutes.`, {
...
export function registerAlertsRoutes(app: Express): void {
    ...
    app.delete("/api/alerts/clear-old", validateJWT, injectOrganizationContext, requireSuperadminRole, validateOrganizationAccess, (req, res) => {
        try {
            ...
            systemAlerts = systemAlerts.filter(alert => new Date(alert.timestamp) > cutoffTime);
    ...
    app.get("/api/alerts/stats", validateJWT, injectOrganizationContext, requireSuperadminRole, validateOrganizationAccess, (req, res) => {
        try {
            ...
            const recent24h = systemAlerts.filter(alert => new Date(alert.timestamp) > last24Hours);
            const recentHour = systemAlerts.filter(alert => new Date(alert.timestamp) > lastHour);
            res.json({
                total: systemAlerts.length,
                last24Hours: {
