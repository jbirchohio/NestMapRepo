...
export const createFeatureFlag = async (req: Request, res: Response) => {
    try {
        ...
        // Log the action
        if (userId) {
            await logSuperadminAction(userId, 'CREATE_FEATURE_FLAG', 'feature_flag', newFlag.id, { name, isEnabled, defaultValue });
        }
...
export const updateFeatureFlag = async (req: Request, res: Response) => {
    try {
        ...
        // Log the action
        if (userId) {
            await logSuperadminAction(userId, 'UPDATE_FEATURE_FLAG', 'feature_flag', id, {
                old: currentFlag,
                new: updatedFlag,
            });
        }
...
export const deleteFeatureFlag = async (req: Request, res: Response) => {
    try {
        ...
        // Log the action
        if (userId) {
            await logSuperadminAction(userId, 'DELETE_FEATURE_FLAG', 'feature_flag', id, { name: flag.name });
        }
...
export const getOrganizationFeatureFlags = async (req: Request, res: Response) => {
    try {
        ...
        const flags = await db
            ...
            .from(superadminFeatureFlags)
            .leftJoin(organizationFeatureFlags, and(eq(organizationFeatureFlags.flag_id, superadminFeatureFlags.id), eq(organizationFeatureFlags.organization_id, organizationId)))
...
export const setOrganizationFeatureFlag = async (req: Request, res: Response) => {
    try {
        ...
        const [existingOverride] = await db
            ...
            .where(and(eq(organizationFeatureFlags.organization_id, organizationId), eq(organizationFeatureFlags.flag_id, flagId)));
        let result;
        if (existingOverride) {
        ...
        // Log the action
        if (userId) {
            await logSuperadminAction(userId, 'SET_ORGANIZATION_FEATURE_FLAG', 'organization_feature_flag', result.id, {
                organizationId,
                flagId,
                flagName: flag.name,
                isEnabled,
...
export const removeOrganizationFeatureFlag = async (req: Request, res: Response) => {
    try {
        ...
        const [deletedOverride] = await db
            .delete(organizationFeatureFlags)
            .where(and(eq(organizationFeatureFlags.organization_id, organizationId), eq(organizationFeatureFlags.flag_id, flag.id)))
            .returning();
        if (!deletedOverride) {
        ...
        // Log the action
        if (userId) {
            await logSuperadminAction(userId, 'REMOVE_ORGANIZATION_FEATURE_FLAG', 'organization_feature_flag', deletedOverride.id, {
                organizationId,
                flagId: flag.id,
                flagName,
            });
