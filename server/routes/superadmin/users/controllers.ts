...
export const getUsers = async (req: Request, res: Response) => {
    try {
        const { page = 1, limit = 50, search = '', role, organizationId, status = 'active' } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
        const searchTerm = `%${search}%`;
        // Build where conditions
        const conditions = [];
        if (search) {
            conditions.push(sql `(LOWER(${users.email}) LIKE LOWER(${searchTerm}) OR 
...
export const createUser = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        await logSuperadminAction(actorId, 'CREATE_USER', 'user', newUser.id, { email, role, organizationId });
...
export const updateUser = async (req: Request, res: Response) => {
    try {
        ...
        // Log the action
        if (currentUserId) {
            await logSuperadminAction(Number(currentUserId), 'UPDATE_USER', 'user', userId, updates);
        }
        else {
...
export const deleteUser = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        if (currentUserId) {
            // TODO: Update audit log service to use string UUIDs instead of numbers
            await logSuperadminAction(1, // Default to system admin ID 1
