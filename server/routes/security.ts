...
router.get('/alerts', async (req: Request, res: Response) => {
    try {
        ...
        const recentLogs = await db.select()
            .from(adminAuditLog)
            .where(and(eq(adminAuditLog.target_organization_id, organizationId), gte(adminAuditLog.timestamp, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)) // Last 7 days
        ))
        ...
        const privilegeChanges = recentLogs.filter(log => log.action_type === 'role_updated' || log.action_type === 'user_role_changed');
        ...
        const adminActions = recentLogs.filter(log => log.action_type.includes('admin') || log.action_type.includes('delete') || log.action_type.includes('role'));
        ...
        const dataAccessActions = recentLogs.filter(log => log.action_type.includes('export') || log.action_type.includes('download'));
...
router.get('/metrics', async (req: Request, res: Response) => {
    try {
        ...
        const [totalUsers, activeUsers24h, adminActions24h, suspiciousActivity7d, failedLogins30d] = await Promise.all([
            ...
            db.select({ count: count() })
                .from(adminAuditLog)
                .where(and(eq(adminAuditLog.target_organization_id, organizationId), gte(adminAuditLog.timestamp, last24h), sql `action_type LIKE '%admin%' OR action_type LIKE '%delete%' OR action_type LIKE '%role%'`)),
            db.select({ count: count() })
                .from(adminAuditLog)
                .where(and(eq(adminAuditLog.target_organization_id, organizationId), gte(adminAuditLog.timestamp, last7d), sql `action_type LIKE '%failed%' OR action_type LIKE '%suspicious%'`)),
            db.select({ count: count() })
                .from(adminAuditLog)
                .where(and(eq(adminAuditLog.target_organization_id, organizationId), gte(adminAuditLog.timestamp, last30d), sql `action_type LIKE '%login_failed%' OR action_type LIKE '%auth_failed%'`))
        ]);
...
router.get('/audit-summary', async (req: Request, res: Response) => {
    ...
    `);
        ...
        const criticalActions = await db.select()
            .from(adminAuditLog)
            .where(and(eq(adminAuditLog.target_organization_id, organizationId), gte(adminAuditLog.timestamp, last7d), sql `action_type IN ('user_deleted', 'role_created', 'role_deleted', 'organization_updated', 'admin_access_granted')`))
            .orderBy(desc(adminAuditLog.timestamp))
            .limit(10);
        const summary = {
...
function calculateSecurityScore(metrics: {
    ...
    if (adminActionRatio > 0.5)
        score -= 20;
    else if (adminActionRatio > 0.3)
        score -= 10;
    else if (adminActionRatio > 0.1)
        score -= 5;
    // Deduct points for suspicious activity
    if (metrics.suspiciousActivity > 10)
        score -= 30;
    else if (metrics.suspiciousActivity > 5)
        score -= 15;
    ...
    if (metrics.failedLogins > 50)
        score -= 25;
    else if (metrics.failedLogins > 20)
        score -= 15;
    else if (metrics.failedLogins > 10)
        score -= 10;
    else if (metrics.failedLogins > 0)
        score -= 5;
