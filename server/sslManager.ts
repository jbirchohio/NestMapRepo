...
export class SSLManager {
    ...
    async createAccount(email: string): Promise<ACMEAccount> {
        ...
        try {
            ...
            const accountUrl = await this.sendACMERequest(directory.newAccount, accountPayload, keyPair.privateKey);
            return {
     ...
     */
    async requestCertificate(domain: string, account: ACMEAccount, validationCallback: (token: string, keyAuthorization: string) => Promise<boolean>): Promise<SSLCertificate> {
        const directoryUrl = this.isProduction ? this.acmeDirectoryUrl : this.stagingDirectoryUrl;
        try {
            // Get ACME directory
            const directoryResponse = await fetch(directoryUrl);
            ...
            const orderUrl = await this.sendACMERequest(directory.newOrder, orderPayload, account.privateKey);
            // Get order details
            const orderResponse = await this.sendACMERequest(typeof orderUrl === 'string' ? orderUrl : await orderUrl.text(), '', account.privateKey);
            ...
            const certificateResponse = await this.sendACMERequest(order.certificate, '', account.privateKey);
     ...
     */
    private async processAuthorization(authUrl: string, account: ACMEAccount, validationCallback: (token: string, keyAuthorization: string) => Promise<boolean>): Promise<void> {
     ...
     */
    private parseCertificateChain(pemData: string): {
        certificate: string;
        chain: string;
    } {
