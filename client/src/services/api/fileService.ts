...
class FileService {
    ...
    public async uploadFile(file: File, metadata?: FileMetadata, onProgress?: (progress: number) => void, config?: RequestConfig): Promise<FileUploadResponse> {
    ...
    public async uploadFiles(files: File[], metadata?: FileMetadata, onProgress?: (progress: number, fileIndex: number, file: File) => void, config?: RequestConfig): Promise<FileUploadResponse[]> {
        const uploadPromises = files.map((file, index) => {
            const fileProgress = (progress: number) => {
                if (onProgress)
                    onProgress(progress, index, file);
            };
    ...
    public async downloadFile(fileId: string, fileName?: string, config?: RequestConfig): Promise<void> {
        const response = await apiClient.get<Blob>(`${this.basePath}/${fileId}/download`, {
            ...config,
            responseType: 'blob',
        });
    ...
    public async generateFileUrl(fileId: string, expiresIn?: number): Promise<string> {
        ...
        const response = await apiClient.get<{
            url: string;
        }>(`${this.basePath}/${fileId}/generate-url`, { params });
        return response.url;
    }
}
