 ...
 * Cache a response
 */
export const cacheResponse = <T>(config: RequestConfig, data: T, headers: Record<string, string> = {}): void => {
    if (!config.useCache)
        return;
    const cacheKey = generateCacheKey(config);
 ...
 */
export const handleDeduplication = <T>(config: RequestConfig, requestFn: () => Promise<AxiosResponse<ApiResponse<T>>>): Promise<AxiosResponse<ApiResponse<T>>> => {
