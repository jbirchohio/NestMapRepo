...
export class ApplicationError extends Error {
    ...
    constructor(message: string, code: string = 'UNKNOWN_ERROR', statusCode: number = 500, isOperational: boolean = true, isSecurityError: boolean = false, securityContext: SecurityContext = {}) {
        super(message);
...
export class SecurityError extends ApplicationError {
    constructor(message: string = 'Security violation detected', code: string = 'SECURITY_ERROR', statusCode: number = 403, isOperational: boolean = true, securityContext: SecurityContext = {}) {
...
export class ErrorHandler {
    ...
    static async handleAsync<T>(operation: () => Promise<T>, context: ErrorContext = {}): Promise<T | null> {
        try {
    ...
    static handleSync<T>(operation: () => T, context: ErrorContext = {}): T | null {
        try {
            return operation();
        }
        catch (error) {
    ...
    static createRetryWrapper<T>(operation: () => Promise<T>, maxRetries: number = 3, delay: number = 1000) {
...
export const withErrorHandling = <T extends any[], R>(fn: (...args: T) => R, context: ErrorContext = {}) => {
    return (...args: T): R | null => {
        return ErrorHandler.handleSync(() => fn(...args), context);
    };
};
export const withAsyncErrorHandling = <T extends any[], R>(fn: (...args: T) => Promise<R>, context: ErrorContext = {}) => {
