...
export default function OrganizationFunding() {
    ...
    const onboardingMutation = useMutation({
        mutationFn: () => apiRequest('POST', '/api/organization-funding/onboarding-link', {
    ...
    const setupFundingMutation = useMutation({
        mutationFn: (data: typeof fundingSource) => apiRequest('POST', '/api/organization-funding/setup-funding', data),
        onSuccess: () => {
            toast({
                title: "Success",
    ...
    if (statusLoading) {
        return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  ...
                  {fundingStatus?.hasStripeAccount ? (<Badge className="bg-green-500 hover:bg-green-600">Connected</Badge>) : (<Badge variant="outline">Not Connected</Badge>)}
                </div>
                <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              ...
              {fundingStatus?.ready ? (<Alert className="border-green-200 bg-green-50 dark:bg-green-900/20">
                  <CheckCircle className="h-4 w-4 text-green-600"/>
                  <AlertDescription className="text-green-800 dark:text-green-200">
                    Your organization is ready to issue corporate cards!
                  </AlertDescription>
                </Alert>) : (<Alert className="border-orange-200 bg-orange-50 dark:bg-orange-900/20">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="status" className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  {accountLoading ? (<div className="flex items-center space-x-2">
                      <Loader2 className="h-4 w-4 animate-spin"/>
                      <span>Checking account status...</span>
                    </div>) : accountStatus ? (<div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span>Account Ready</span>
                        {accountStatus.ready ? (<Badge className="bg-green-500 hover:bg-green-600">Ready</Badge>) : (<Badge variant="outline">Pending</Badge>)}
                      </div>
                      {accountStatus.status && (<div className="flex items-center justify-between">
                          <span>Issuing Capability</span>
                          {getStatusBadge(accountStatus.status)}
                        </div>)}
                      {/* Enhanced verification requirements display */}
                      {fundingStatus && (<div className="space-y-3">
                          {/* Currently due requirements */}
                          {fundingStatus.requirementsCurrentlyDue?.length > 0 && (<Alert className="border-red-200 bg-red-50 dark:bg-red-900/20">
                              <AlertCircle className="h-4 w-4 text-red-600"/>
                              <AlertDescription className="text-red-800 dark:text-red-200">
                                <div className="font-semibold mb-2">Action Required:</div>
                                <ul className="list-disc pl-4 space-y-1">
                                  {fundingStatus.requirementsCurrentlyDue.map((req, index) => (<li key={index} className="text-sm">{req}</li>))}
                                </ul>
                                {fundingStatus.requirementsCurrentDeadline && (<div className="mt-2 text-sm">
                          ...
                          {fundingStatus.requirementsPastDue?.length > 0 && (<Alert className="border-red-600 bg-red-100 dark:bg-red-900/30">
                              <AlertCircle className="h-4 w-4 text-red-700"/>
                              <AlertDescription className="text-red-900 dark:text-red-100">
                                <div className="font-semibold mb-2">Past Due - Immediate Action Required:</div>
                                <ul className="list-disc pl-4 space-y-1">
                                  {fundingStatus.requirementsPastDue.map((req, index) => (<li key={index} className="text-sm">{req}</li>))}
                                </ul>
                              </AlertDescription>
                            </Alert>)}

                          {/* Eventually due requirements */}
                          {fundingStatus.requirementsEventuallyDue?.length > 0 && (<Alert className="border-yellow-200 bg-yellow-50 dark:bg-yellow-900/20">
                              <AlertCircle className="h-4 w-4 text-yellow-600"/>
                              <AlertDescription className="text-yellow-800 dark:text-yellow-200">
                                <div className="font-semibold mb-2">Eventually Required:</div>
                                <ul className="list-disc pl-4 space-y-1">
                                  {fundingStatus.requirementsEventuallyDue.map((req, index) => (<li key={index} className="text-sm">{req}</li>))}
                                </ul>
                              </AlertDescription>
                            </Alert>)}

                          {/* Verification errors */}
                          {fundingStatus.errors?.length > 0 && (<Alert className="border-red-200 bg-red-50 dark:bg-red-900/20">
                              <AlertCircle className="h-4 w-4 text-red-600"/>
                              <AlertDescription className="text-red-800 dark:text-red-200">
                                <div className="font-semibold mb-2">Verification Errors:</div>
                                <div className="space-y-2">
                                  {fundingStatus.errors.map((error, index) => (<div key={index} className="bg-white dark:bg-slate-800 p-3 rounded border">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              ...
                              {fundingStatus.paymentsEnabled ? (<Badge className="bg-green-500 hover:bg-green-600">Enabled</Badge>) : (<Badge variant="outline">Disabled</Badge>)}
                            </div>
                            <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              <span className="text-sm font-medium">Payouts</span>
                              {fundingStatus.chargesEnabled ? (<Badge className="bg-green-500 hover:bg-green-600">Enabled</Badge>) : (<Badge variant="outline">Disabled</Badge>)}
                            </div>
                            <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                              <span className="text-sm font-medium">Transfers</span>
                              {fundingStatus.transfersEnabled ? (<Badge className="bg-green-500 hover:bg-green-600">Enabled</Badge>) : (<Badge variant="outline">Disabled</Badge>)}
                            </div>
                          </div>

                          {/* Account disabled reason */}
                          {fundingStatus.requirementsDisabledReason && (<Alert className="border-red-200 bg-red-50 dark:bg-red-900/20">
                              <AlertCircle className="h-4 w-4 text-red-600"/>
                              <AlertDescription className="text-red-800 dark:text-red-200">
                    ...
                    </div>) : fundingStatus?.hasStripeAccount ? (<p className="text-slate-600">Unable to fetch account status</p>) : (<p className="text-slate-600">No Stripe account connected</p>)}
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="account" className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  {fundingStatus?.hasStripeAccount ? (<Alert className="border-green-200 bg-green-50">
                      <CheckCircle className="h-4 w-4 text-green-600"/>
                      <AlertDescription>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input id="businessName" value={businessInfo.businessName} onChange={(e) => setBusinessInfo(prev => ({ ...prev, businessName: e.target.value }))} placeholder="Your Organization Name"/>
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="email">Business Email</Label>
                          <Input id="email" type="email" value={businessInfo.email} onChange={(e) => setBusinessInfo(prev => ({ ...prev, email: e.target.value }))} placeholder="admin@yourcompany.com"/>
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="country">Country</Label>
                          <Select value={businessInfo.country} onValueChange={(value) => setBusinessInfo(prev => ({ ...prev, country: value }))}>
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <div className="space-y-2">
                          ...
                          <Select value={businessInfo.currency} onValueChange={(value) => setBusinessInfo(prev => ({ ...prev, currency: value }))}>
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                      ...
                      <Button onClick={() => createAccountMutation.mutate()} disabled={createAccountMutation.isPending} className="w-full bg-electric-600 hover:bg-electric-700">
            ...
            <TabsContent value="onboarding" className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  {!fundingStatus?.hasStripeAccount ? (<Alert className="border-orange-200 bg-orange-50">
                      <AlertCircle className="h-4 w-4 text-orange-600"/>
                      <AlertDescription>
                        You must create a Stripe Connect account first.
                      </AlertDescription>
                    </Alert>) : accountStatus?.ready ? (<Alert className="border-green-200 bg-green-50">
                      <CheckCircle className="h-4 w-4 text-green-600"/>
                      <AlertDescription>
                      ...
                      <Button onClick={() => onboardingMutation.mutate()} disabled={onboardingMutation.isPending} className="w-full bg-electric-600 hover:bg-electric-700">
            ...
            <TabsContent value="funding" className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  {!fundingStatus?.issuingEnabled ? (<Alert className="border-orange-200 bg-orange-50">
                      <AlertCircle className="h-4 w-4 text-orange-600"/>
                      <AlertDescription>
                        Complete Stripe onboarding before configuring funding sources.
                      </AlertDescription>
                    </Alert>) : fundingStatus?.fundingSourceStatus === 'active' ? (<Alert className="border-green-200 bg-green-50">
                      <CheckCircle className="h-4 w-4 text-green-600"/>
                      <AlertDescription>
                      ...
                      {fundingSource.type === 'bank_account' && (<div className="space-y-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                          <h4 className="font-medium">Bank Account Details</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <Label htmlFor="accountHolderName">Account Holder Name</Label>
                              <Input id="accountHolderName" value={fundingSource.bankAccount.accountHolderName} onChange={(e) => setFundingSource(prev => ({
                    ...prev,
                    ...
                    bankAccount: { ...prev.bankAccount, accountType: value }
                            ...
                            <div className="space-y-2">
                              ...
                              <Input id="routingNumber" value={fundingSource.bankAccount.routingNumber} onChange={(e) => setFundingSource(prev => ({
                    ...prev,
                    bankAccount: { ...prev.bankAccount, routingNumber: e.target.value }
                            ...
                            <div className="space-y-2">
                              ...
                              <Input id="accountNumber" value={fundingSource.bankAccount.accountNumber} onChange={(e) => setFundingSource(prev => ({
                    ...prev,
                    bankAccount: { ...prev.bankAccount, accountNumber: e.target.value }
                      ...
                      {fundingSource.type === 'credit_line' && (<div className="space-y-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                          <h4 className="font-medium">Credit Line Request</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <Label htmlFor="requestedAmount">Requested Amount</Label>
                              <Input id="requestedAmount" type="number" value={fundingSource.creditLine.requestedAmount} onChange={(e) => setFundingSource(prev => ({
                    ...prev,
                    creditLine: { ...prev.creditLine, requestedAmount: Number(e.target.value) }
                            ...
                            <div className="space-y-2">
                              <Label htmlFor="currency">Currency</Label>
                              <Select value={fundingSource.creditLine.currency} onValueChange={(value) => setFundingSource(prev => ({
                    ...prev,
                    creditLine: { ...prev.creditLine, currency: value }
                      ...
                      <Button onClick={() => setupFundingMutation.mutate(fundingSource)} disabled={setupFundingMutation.isPending} className="w-full bg-electric-600 hover:bg-electric-700">
