...
import { HelpCircle, MessageSquare, BookOpen, Users, CreditCard, Map, Brain, Shield, Search, Mail, Phone, Globe, ChevronRight, Sparkles } from 'lucide-react';
        ...
        <Tabs defaultValue="faq" className="space-y-8">
          ...
          <TabsContent value="faq" className="space-y-6">
            {filteredFAQs.length === 0 ? (<Card>
                <CardContent className="text-center py-12">
              ...
              </Card>) : (filteredFAQs.map((category, index) => (<Card key={index}>
                  <CardHeader>
                  ...
                  <CardContent>
                    <Accordion type="single" collapsible>
                      {category.questions.map((qa, qaIndex) => (<AccordionItem key={qaIndex} value={`item-${index}-${qaIndex}`}>
                  ...
                  <form onSubmit={handleContactSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input value={contactForm.name} onChange={(e) => setContactForm(prev => ({ ...prev, name: e.target.value }))} required/>
                      </div>
                      <div>
                        <label className="text-sm font-medium">Email</label>
                        <Input type="email" value={contactForm.email} onChange={(e) => setContactForm(prev => ({ ...prev, email: e.target.value }))} required/>
                      </div>
                    </div>

                    <div>
                      <label className="text-sm font-medium">Category</label>
                      <select className="w-full mt-1 p-2 border rounded-md" value={contactForm.category} onChange={(e) => setContactForm(prev => ({ ...prev, category: e.target.value }))}>
                    ...
                    <div>
                      ...
                      <Input value={contactForm.subject} onChange={(e) => setContactForm(prev => ({ ...prev, subject: e.target.value }))} required/>
                    </div>

                    <div>
                      <label className="text-sm font-medium">Message</label>
                      <Textarea rows={5} value={contactForm.message} onChange={(e) => setContactForm(prev => ({ ...prev, message: e.target.value }))} placeholder="Describe your issue or question in detail..." required/>
                    </div>
