...
import { Shield, Settings2, Palette, Crown, CheckCircle, Building2, Globe, RotateCcw, Image, Eye, Save, Upload, Paintbrush, Type } from "lucide-react";
...
export default function Settings() {
    ...
    const hasSettingsAccess = userPermissions && ((userPermissions as any).canManageOrganization ||
    ...
    if (showOnboarding && orgPlan) {
        return (<BrandingOnboarding onComplete={() => setShowOnboarding(false)} onSkip={() => setShowOnboarding(false)} organizationPlan={orgPlan.plan}/>);
    }
            ...
            <Tabs defaultValue="branding" className="space-y-6">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <CardContent className="space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <Input id="companyName" {...form.register('companyName')} disabled={!canUseBranding} placeholder="Your Company Name"/>
                          {form.formState.errors.companyName && (<p className="text-sm text-red-500">{form.formState.errors.companyName.message}</p>)}
                        </div>
                        
                        <div className="space-y-2">
                          <Label htmlFor="tagline">Tagline</Label>
                          <Input id="tagline" {...form.register('tagline')} disabled={!canUseBranding} placeholder="Your company tagline"/>
                        </div>
                      </div>
                        ...
                        <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6">
                          <div className="text-center">
                            ...
                            <div className="mt-4">
                              ...
                              <Input id="logo-upload" type="file" accept="image/*" className="hidden" disabled={!canUseBranding}/>
                            </div>
                        ...
                        <div className="grid md:grid-cols-3 gap-4">
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Input id="primaryColor" type="color" value={form.watch('primaryColor')} onChange={(e) => form.setValue('primaryColor', e.target.value)} disabled={!canUseBranding} className="w-16 h-10 p-1 border rounded"/>
                              <Input value={form.watch('primaryColor')} onChange={(e) => form.setValue('primaryColor', e.target.value)} disabled={!canUseBranding} placeholder="#6D5DFB"/>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Input id="secondaryColor" type="color" value={form.watch('secondaryColor')} onChange={(e) => form.setValue('secondaryColor', e.target.value)} disabled={!canUseBranding} className="w-16 h-10 p-1 border rounded"/>
                              <Input value={form.watch('secondaryColor')} onChange={(e) => form.setValue('secondaryColor', e.target.value)} disabled={!canUseBranding} placeholder="#6D5DFB"/>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Input id="accentColor" type="color" value={form.watch('accentColor')} onChange={(e) => form.setValue('accentColor', e.target.value)} disabled={!canUseBranding} className="w-16 h-10 p-1 border rounded"/>
                              <Input value={form.watch('accentColor')} onChange={(e) => form.setValue('accentColor', e.target.value)} disabled={!canUseBranding} placeholder="#6D5DFB"/>
                            </div>
                          </div>
                        </div>
                        
                        {/* Reset to Default Button */}
                        {canUseBranding && (<div className="pt-4 border-t">
                ...
                form.setValue('accentColor', '#6D5DFB');
                      ...
                      {!canUseBranding && (<Alert>
                          <Crown className="h-4 w-4"/>
                          <AlertDescription>
                            White label branding requires Pro plan ($99/month) or higher. 
                            <Button variant="link" className="p-0 h-auto font-semibold text-electric-600" onClick={() => setShowOnboarding(true)}>
                              Upgrade now
                            </Button>
                          </AlertDescription>
                        </Alert>)}
                    ...
                    <CardContent className="space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <Input id="customDomain" {...form.register('customDomain')} disabled={!canUseBranding || orgPlan?.plan === 'pro'} placeholder="your-company.com"/>
                          {orgPlan?.plan === 'pro' && (<p className="text-sm text-amber-600">Custom domain available in Business plan</p>)}
                        </div>
                        
                        <div className="space-y-2">
                          <Label htmlFor="supportEmail">Support Email</Label>
                          <Input id="supportEmail" type="email" {...form.register('supportEmail')} disabled={!canUseBranding} placeholder="support@yourcompany.com"/>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                {canUseBranding && (<div className="flex justify-end">
                    <Button type="submit" disabled={saveMutation.isPending} className="gap-2">
                      {saveMutation.isPending ? (<div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"/>) : (<Save className="h-4 w-4"/>)}
                      Save Settings
                    </Button>
                  </div>)}
              </form>
