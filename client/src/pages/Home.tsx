...
export default function Home() {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-white via-electric-50/30 to-electric-100/50 dark:from-dark-900 dark:via-electric-900/10 dark:to-electric-800/20">
      {/* Handle automatic routing for authenticated users */}
      {user && authReady && <RoleBasedRedirect />}
      <NewTripModal isOpen={isNewTripModalOpen} onClose={() => setIsNewTripModalOpen(false)} onSuccess={handleTripCreated} userId={effectiveUserId} isGuestMode={isGuestMode}/>
      
      <AuthModal isOpen={isAuthModalOpen} onClose={() => setIsAuthModalOpen(false)} defaultView={authView} onSuccess={() => {
          ...
          <div className="container mx-auto px-4 py-16 sm:py-24">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center">
              <div className="flex justify-center mb-8">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="h-20 w-20 bg-gradient-to-br from-electric-400 to-electric-600 rounded-2xl flex items-center justify-center shadow-lg shadow-electric-500/25">
                  <Plane className="h-10 w-10 text-white"/>
                </motion.div>
              </div>
              
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="text-5xl sm:text-6xl lg:text-7xl font-bold bg-gradient-to-r from-electric-600 via-electric-500 to-electric-700 bg-clip-text text-transparent mb-6">
                Travel Reimagined
              </motion.h1>
              
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="text-xl sm:text-2xl text-electric-700 dark:text-electric-300 mb-4 max-w-3xl mx-auto">
                The AI-powered corporate travel platform that transforms how teams plan, book, and manage business travel
              </motion.p>
              
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5 }} className="text-lg text-electric-600/80 dark:text-electric-400/80 mb-12 max-w-2xl mx-auto">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                {user ? (<PrimaryButton onClick={handleCreateNewTrip} className="text-lg px-8 py-4">
                    <Plus className="mr-2 h-5 w-5"/>
                    Start New Trip
                  </PrimaryButton>) : (<div className="flex flex-col sm:flex-row gap-4">
                    <PrimaryButton onClick={handleSignUpClick} className="text-lg px-8 py-4">
                      <Sparkles className="mr-2 h-5 w-5"/>
                      Get Started
                    </PrimaryButton>
                    <Button onClick={handleSignInClick} variant="outline" className="text-lg px-8 py-4 border-electric-300 text-electric-700 hover:bg-electric-50 dark:border-electric-600 dark:text-electric-300 dark:hover:bg-electric-900/20">
                      Sign In
                    </Button>
                  </div>)}
              </motion.div>
              
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }} className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
      ...
      <div className="container mx-auto px-4 py-8">
        {user && (<div className="mb-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-2">
                <Button onClick={handleSignOut} variant="ghost" size="sm" className="text-electric-600 hover:text-electric-700 hover:bg-electric-50 dark:text-electric-400 dark:hover:text-electric-300 dark:hover:bg-electric-900/20">
                  <LogOut className="mr-2 h-4 w-4"/>
                  Sign Out
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setLocation('/analytics')} title="Analytics Dashboard" className="text-electric-600 hover:text-electric-700 hover:bg-electric-50 dark:text-electric-400 dark:hover:text-electric-300 dark:hover:bg-electric-900/20">
                  <BarChart3 className="h-4 w-4 mr-2"/>
                  Analytics
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setLocation('/team')} title="Team Management" className="text-electric-600 hover:text-electric-700 hover:bg-electric-50 dark:text-electric-400 dark:hover:text-electric-300 dark:hover:bg-electric-900/20">
                  <UserRound className="h-4 w-4 mr-2"/>
                  Team
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setLocation('/billing')} title="Billing & Subscription" className="text-electric-600 hover:text-electric-700 hover:bg-electric-50 dark:text-electric-400 dark:hover:text-electric-300 dark:hover:bg-electric-900/20">
        ...
        {tripsQuery.isLoading ? (<div className="flex justify-center items-center py-12">
            <div className="animate-spin w-8 h-8 border-4 border-electric-500 border-t-transparent rounded-full"/>
          </div>) : trips && trips.length > 0 ? (<div className="space-y-8">
            <Tabs defaultValue="active" className="w-full">
              ...
              <TabsContent value="all" className="space-y-4">
                {trips.map((trip) => (<SwipeableTrip key={trip.id} trip={trip} onNavigate={handleNavigateToTrip} onRename={handleOpenRenameDialog}/>))}
              </TabsContent>
            </Tabs>
        ...
        {!user && (<div className="mt-16 bg-white/80 dark:bg-dark-800/80 backdrop-blur-sm border border-electric-200/50 dark:border-electric-700/50 rounded-xl p-8">
            <TripTemplates userId={effectiveUserId} onTripCreated={() => {
                setAuthView("signup");
                setIsAuthModalOpen(true);
            }}/>
          </div>)}
      ...
      {tripToRename && (<RenameTripDialog isOpen={isRenameModalOpen} onClose={handleCloseRenameDialog} trip={tripToRename}/>)}
    </div>);
}
