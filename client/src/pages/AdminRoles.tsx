...
import { Shield, Users, Plus, Edit, Trash2, Save, User, Settings, Eye, Lock } from "lucide-react";
interface Role {
    id: string;
    name: string;
    description: string;
...
export default function AdminRoles() {
    ...
    const updateRoleMutation = useMutation({
        mutationFn: async ({ id, ...roleData }: Partial<Role> & {
            id: string;
        }) => {
    ...
    const handleSaveRole = () => {
        if (editingRole.id) {
            updateRoleMutation.mutate(editingRole as Role & {
                id: string;
            });
        }
        else {
    ...
    if (isLoading) {
        return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-electric-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-6">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-electric-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <Dialog open={isCreating} onOpenChange={setIsCreating}>
              <DialogTrigger asChild>
                <Button className="bg-electric-600 hover:bg-electric-700" onClick={() => {
            setIsCreating(true);
            setEditingRole({ permissions: [] });
              ...
              <DialogContent className="max-w-2xl">
                ...
                <RoleEditor role={editingRole} permissions={permissions} onRoleChange={setEditingRole} onPermissionToggle={handlePermissionToggle} onSave={handleSaveRole} onCancel={() => {
            setIsCreating(false);
            setEditingRole({});
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {roles.map((role, index) => (<motion.div key={role.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
              <Card className="h-full hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-electric-100 dark:bg-electric-900/20 rounded-lg">
                        {role.isSystemRole ? (<Shield className="w-5 h-5 text-electric-600"/>) : (<User className="w-5 h-5 text-electric-600"/>)}
                      </div>
                      <div>
                        <CardTitle className="text-lg">{role.name}</CardTitle>
                        {role.isSystemRole && (<Badge variant="secondary" className="mt-1 flex items-center gap-1">
                ...
                setSelectedRole(role);
                      ...
                      {!role.isSystemRole && (<Button variant="ghost" size="sm" onClick={() => deleteRoleMutation.mutate(role.id)} disabled={deleteRoleMutation.isPending}>
                ...
                return (<Badge key={permissionId} variant="secondary" className="text-xs">
        ...
        <Dialog open={!!selectedRole} onOpenChange={() => setSelectedRole(null)}>
          <DialogContent className="max-w-2xl">
            ...
            {selectedRole && (<RoleEditor role={editingRole} permissions={permissions} onRoleChange={setEditingRole} onPermissionToggle={handlePermissionToggle} onSave={handleSaveRole} onCancel={() => setSelectedRole(null)} isLoading={updateRoleMutation.isPending} isSystemRole={selectedRole.isSystemRole}/>)}
          </DialogContent>
        </Dialog>
      </div>
    </div>);
...
function RoleEditor({ role, permissions, onRoleChange, onPermissionToggle, onSave, onCancel, isLoading, isSystemRole }: RoleEditorProps) {
    ...
    return (<div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input id="roleName" value={role.name || ''} onChange={(e) => onRoleChange({ ...role, name: e.target.value })} disabled={isSystemRole} placeholder="Enter role name"/>
        </div>
        <div>
          <Label htmlFor="roleDescription">Description</Label>
          <Input id="roleDescription" value={role.description || ''} onChange={(e) => onRoleChange({ ...role, description: e.target.value })} placeholder="Enter role description"/>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mt-3 space-y-4">
          {Object.entries(permissionsByCategory).map(([category, categoryPermissions]) => (<div key={category} className="border rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                {categoryPermissions.map(permission => (<div key={permission.id} className="flex items-start space-x-3">
                    <Checkbox id={permission.id} checked={role.permissions?.includes(permission.id) || false} onCheckedChange={(checked) => onPermissionToggle(permission.id, checked as boolean)} disabled={isSystemRole}/>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        ...
        <Button onClick={onSave} disabled={isLoading || !role.name || isSystemRole} className="bg-electric-600 hover:bg-electric-700">
