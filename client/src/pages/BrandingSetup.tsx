...
import { Crown, ArrowLeft, CheckCircle, Sparkles, Building2, Palette, Eye } from 'lucide-react';
...
export default function BrandingSetup() {
    ...
    if (onboardingLoading) {
        return (<div className="min-h-screen bg-gradient-to-br from-navy-50 to-soft-100 dark:from-navy-900 dark:to-navy-800 flex items-center justify-center">
    ...
    if (showOnboarding && orgPlan) {
        return (<BrandingOnboarding onComplete={handleBrandingComplete} onSkip={handleSkipBranding} organizationPlan={orgPlan.plan}/>);
    }
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-navy-50 to-soft-100 dark:from-navy-900 dark:to-navy-800">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          {/* Pro Plan Status */}
          {canUseBranding ? (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            </motion.div>) : (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          {/* Feature Comparison */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Basic Plan */}
            <Card className={`border-gray-200 dark:border-gray-700 ${orgPlan?.plan === 'basic' ? 'bg-gray-50 dark:bg-gray-800/50' : ''}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Basic ($29/month)
                  {orgPlan?.plan === 'basic' && (<Badge variant="secondary" className="ml-auto">
                      Current
                    </Badge>)}
                </CardTitle>
              </CardHeader>
            ...
            <Card className={`border-electric-200 ${orgPlan?.plan === 'pro' ? 'bg-gradient-to-br from-electric-50 to-electric-100 dark:from-electric-900/20 dark:to-electric-800/20' : ''}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Pro ($99/month)
                  {orgPlan?.plan === 'pro' && (<Badge variant="default" className="ml-auto">
            ...
            <Card className={`border-emerald-200 ${orgPlan?.plan === 'business' ? 'bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20' : ''}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Business ($199/month)
                  {orgPlan?.plan === 'business' && (<Badge variant="default" className="ml-auto">
                      <CheckCircle className="h-3 w-3 mr-1"/>
                      Current
                    </Badge>)}
                </CardTitle>
            ...
            <Card className={`border-orange-200 ${orgPlan?.plan === 'enterprise' ? 'bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20' : ''}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {orgPlan?.plan === 'enterprise' && (<Badge variant="default" className="ml-auto">
                      <CheckCircle className="h-3 w-3 mr-1"/>
                      Current
                    </Badge>)}
                </CardTitle>
          ...
          {/* Action Section */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-center space-y-6">
            {canUseBranding ? (<div className="space-y-4">
                <div className="p-6 bg-electric-50 dark:bg-electric-900/20 rounded-lg border border-electric-200 dark:border-electric-800">
                  ...
                  <Button onClick={handleStartBranding} className="bg-electric-600 hover:bg-electric-700 gap-2">
                ...
                <div className="p-6 bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-lg border border-orange-200 dark:border-orange-800">
                  ...
                  <Button onClick={handleUpgradeToProfessional} disabled={autoEnableMutation.isPending} className="bg-orange-600 hover:bg-orange-700 gap-2">
                    {autoEnableMutation.isPending ? (<div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"/>) : (<Crown className="h-4 w-4"/>)}
