...
export default function TripPlanner() {
    ...
    // Fetch trip data
    const { trip, todos, notes, isLoading: isTripLoading, error: tripError } = useTrip(tripId);
    // Fetch activities
    const { activities, isLoading: isActivitiesLoading, error: activitiesError, refetchActivities } = useActivities(tripId);
    ...
    const mapCenter = mapMarkers.length > 0
        ...
        : (trip?.cityLatitude && trip?.cityLongitude &&
            ...
            ? [parseFloat(trip.cityLongitude), parseFloat(trip.cityLatitude)] as [
                number,
                number
            ]
            : [-74.006, 40.7128] as [
                number,
                number
            ]; // Default to NYC coordinates instead of undefined
    ...
    return (<AppShell trip={trip} onOpenShare={handleOpenShare}>
      ...
      <div className="md:hidden flex border rounded-md overflow-hidden shadow-sm m-2 relative z-50 bg-white dark:bg-[hsl(var(--card))]">
        <Button onClick={() => setMobileView('itinerary')} variant={mobileView === 'itinerary' ? 'default' : 'outline'} className="flex-1 py-3 px-4 font-medium h-auto rounded-r-none">
          Itinerary
        </Button>
        <Button onClick={() => setMobileView('map')} variant={mobileView === 'map' ? 'default' : 'outline'} className="flex-1 py-3 px-4 font-medium h-auto rounded-l-none">
          Map
        </Button>
      </div>
      ...
      <div className="hidden md:grid h-[calc(100vh-110px)]" style={{ gridTemplateColumns: '400px calc(100vw - 400px)' }}>
        <div className="h-full overflow-y-auto">
          <ItinerarySidebar trip={trip} activities={activities} todos={todos} notes={notes} activeDay={activeDay} onChangeDayClick={setActiveDay} onActivitiesUpdated={refetchActivities} onAddActivity={handleOpenActivityModal} mobileView={mobileView} setMobileView={setMobileView}/>
        </div>
        
        <div className="h-full">
          <MapView markers={mapMarkers} routes={mapRoutes} center={mapCenter} zoom={13} onMarkerClick={handleMarkerClick}/>
        </div>
      </div>
      ...
      <div className="md:hidden">
        {/* Itinerary view */}
        {mobileView === 'itinerary' && (<div className="h-[calc(100vh-120px)] overflow-y-auto">
            <ItinerarySidebar trip={trip} activities={activities} todos={todos} notes={notes} activeDay={activeDay} onChangeDayClick={setActiveDay} onActivitiesUpdated={refetchActivities} onAddActivity={handleOpenActivityModal} mobileView={mobileView} setMobileView={setMobileView}/>
          </div>)}
      </div>
      
      {/* Mobile map view - must be outside other containers to be full screen */}
      {mobileView === 'map' && (<div className="md:hidden fixed inset-0 top-[108px] z-10" style={{ height: 'calc(100vh - 108px)' }}>
          <MapView markers={mapMarkers} routes={mapRoutes} center={mapCenter} zoom={13} onMarkerClick={handleMarkerClick}/>
        </div>)}
      
      {/* Share Trip Modal */}
      <ShareTripModal isOpen={shareModalOpen} onClose={() => setShareModalOpen(false)} trip={trip} onSave={handleSaveShareSettings}/>
      
      {/* Centralized Activity Modal */}
      {isActivityModalOpen && activeDay && (<ActivityModal onClose={handleCloseActivityModal} tripId={tripId} date={activeDay} activity={selectedActivity} onSave={handleActivitySaved}/>)}
    </AppShell>);
}
