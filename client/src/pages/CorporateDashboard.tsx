...
import { Building2, Users, Calendar, TrendingUp, MapPin, DollarSign, Plane, Clock, Plus, BarChart3, Settings, Sparkles, CreditCard, Lock, Unlock, Eye } from "lucide-react";
...
export default function CorporateDashboard() {
    ...
    const { data: analytics } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.ok)
                return { totalTrips: 0, totalBudget: 0, avgDuration: 0, teamSize: 0 };
    ...
    const avgTripDuration = trips.length > 0 ? Math.round(trips.reduce((sum, trip) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-navy-50 to-soft-100 dark:from-navy-900 dark:to-navy-800">
      <div className="container mx-auto px-4 py-8">
        {/* Hero Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative overflow-hidden bg-gradient-to-br from-electric-500 via-electric-600 to-electric-700 text-white mb-8">
          ...
          <div className="relative container mx-auto px-6 py-16">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="flex flex-col sm:flex-row gap-4">
                <Button onClick={() => setIsNewTripModalOpen(true)} className="bg-white hover:bg-white/90 text-electric-600 font-semibold px-8 py-3 h-auto rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 electric-glow" size="lg">
        ...
        {trips.length === 0 && (<div className="mb-8">
            <OnboardingProgress onTaskClick={handleOnboardingTaskClick}/>
          </div>)}

        {/* Quick Stats */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <AnimatedCard variant="glow" className="p-6">
            ...
            <CardContent>
              {tripsLoading ? (<div className="space-y-4">
                ...
                </div>) : recentTrips.length > 0 ? (<div className="space-y-4">
                  {recentTrips.map((trip) => (<div key={trip.id} className="flex items-center justify-between p-4 border rounded-lg cursor-pointer hover:shadow-md hover:bg-muted/50 transition-all" onClick={() => setLocation(`/trip/${trip.id}`)}>
          ...
          <AnimatedCard variant="glow" className="p-6">
            ...
            <CardContent>
              {upcomingTrips.length > 0 ? (<div className="space-y-4">
                  {upcomingTrips.map((trip) => (<div key={trip.id} className="flex items-center justify-between p-4 border rounded-lg cursor-pointer hover:shadow-md hover:bg-muted/50 transition-all" onClick={() => setLocation(`/trip/${trip.id}`)}>
                      <div className="flex items-center gap-3">
        ...
        <AnimatedCard variant="glow" className="p-6">
          ...
          <CardContent>
            {cardsLoading ? (<div className="space-y-4">
                {[1, 2, 3].map((i) => (<div key={i} className="h-24 bg-muted animate-pulse rounded"/>))}
              </div>) : corporateCards?.cards?.length > 0 ? (<div className="space-y-4">
                {corporateCards.cards.slice(0, 3).map((card: CorporateCard) => (<div key={card.id} className="flex items-center justify-between p-4 border rounded-lg cursor-pointer hover:shadow-md hover:bg-muted/50 transition-all" onClick={() => setSelectedCard(card)}>
                    <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                      <div className="flex items-center gap-2">
                        ...
                        {card.status === 'active' ? (<Button size="sm" variant="outline" onClick={(e) => {
                        e.stopPropagation();
                        freezeCardMutation.mutate(card.id);
                    }} disabled={freezeCardMutation.isPending}>
                            <Lock className="h-3 w-3"/>
                          </Button>) : (<Button size="sm" variant="outline" onClick={(e) => {
                        e.stopPropagation();
                ...
                {corporateCards.cards.length > 3 && (<div className="text-center pt-2">
      ...
      {/* Card Details Modal */}
      {selectedCard && (<Dialog open={!!selectedCard} onOpenChange={() => setSelectedCard(null)}>
      ...
      <Dialog open={showAddFunds} onOpenChange={setShowAddFunds}>
        ...
        }} disabled={addFundsMutation.isPending || !addFundsAmount} className="flex-1">
