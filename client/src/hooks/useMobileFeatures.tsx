    ...
    const setTravelModeWithEffects = (enabled: boolean) => {
        ...
        if (enabled) {
            ...
            if (navigator.geolocation && !isLocationEnabled) {
                navigator.geolocation.getCurrentPosition(() => setIsLocationEnabled(true), (err) => setLocationError(err.message));
            }
            // Request notification permission
            requestNotificationPermission();
            // Send welcome notification
            setTimeout(() => {
                sendNotification('Travel Mode Active', 'NestMap is now tracking your journey and will send helpful updates!');
            }, 1000);
        }
    };
