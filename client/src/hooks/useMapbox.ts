...
export default function useMapbox() {
    ...
    const initializeMap = useCallback(async (container: HTMLDivElement, center: [
        number,
        number
    ...
    const addRoutes = useCallback((routes: MapRoute[]) => {
        if (!mapInstance.current || !routes.length)
            return;
    ...
    const flyToLocation = useCallback((center: [
        number,
        number
    ], zoom?: number) => {
        if (!mapInstance.current)
