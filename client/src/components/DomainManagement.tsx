...
export default function DomainManagement() {
    ...
    return (<div className="space-y-6">
      <Card>
                ...
                <Tabs value={domainType} onValueChange={(v) => setDomainType(v as 'custom' | 'subdomain')}>
                  ...
                  <TabsContent value="subdomain" className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Input id="subdomain" placeholder="mycompany" value={subdomain} onChange={(e) => setSubdomain(e.target.value)}/>
                  ...
                  <TabsContent value="custom" className="space-y-4">
                    <div>
                      ...
                      <Input id="custom-domain" placeholder="travel.mycompany.com" value={customDomain} onChange={(e) => setCustomDomain(e.target.value)}/>
                    </div>
                  </TabsContent>
                </Tabs>
                <DialogFooter>
        ...
        <CardContent>
          {dashboard.domains.length === 0 ? (<div className="text-center py-8">
              ...
              {dashboard.domains.map((domain) => (<Card key={domain.id}>
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            ...
                            {domain.dns_verified && (<Badge variant="outline" className="text-green-600">
                                <CheckCircle className="h-3 w-3 mr-1"/>
                                DNS Verified
                              </Badge>)}
                            {domain.ssl_verified && (<Badge variant="outline" className="text-green-600">
                                <CheckCircle className="h-3 w-3 mr-1"/>
                                SSL Active
                              </Badge>)}
                          </div>
                      ...
                      <div className="flex items-center space-x-2">
                        {domain.status === 'pending' && (<Button size="sm" onClick={() => verifyDomainMutation.mutate(domain.id)} disabled={verifyDomainMutation.isPending}>
                            Verify
                          </Button>)}
                        {domain.status === 'active' && (<Button size="sm" variant="outline" asChild>
                            <a href={`https://${domain.domain || `${domain.subdomain}.nestmap.com`}`} target="_blank" rel="noopener noreferrer">
                              <ExternalLink className="h-4 w-4 mr-1"/>
                              Visit
                            </a>
                          </Button>)}
                        <Button size="sm" variant="outline" onClick={() => deleteDomainMutation.mutate(domain.id)}>
                          <Trash2 className="h-4 w-4"/>
                        </Button>
                      </div>
                    </div>
                    {domain.domain && domain.status === 'pending' && (<Alert className="mt-4">
                        <AlertCircle className="h-4 w-4"/>
                        <AlertDescription>
                          <div className="space-y-2">
                            ...
                            <div className="bg-gray-100 p-2 rounded font-mono text-sm flex items-center justify-between">
                              ...
                              <Button size="sm" variant="ghost" onClick={() => copyToClipboard(`${domain.domain} CNAME your-nestmap-domain.com`)}>
