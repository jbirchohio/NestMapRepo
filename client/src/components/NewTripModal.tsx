    ...
    return (<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]" style={{
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="title" {...register("title")} placeholder="e.g., Summer Vacation"/>
            ...
            <div className="grid gap-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Switch id="tripType" checked={tripType === "business"} onCheckedChange={(checked) => setValue("tripType", checked ? "business" : "personal")}/>
            ...
            {tripType === "business" && (<div className="space-y-4 p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input id="organization" {...register("organization")} placeholder="e.g., Sales Team, Marketing Dept"/>
                  </div>
                  
                  <div className="grid gap-2">
                    <Label htmlFor="budget">Budget (Optional)</Label>
                    <Input id="budget" {...register("budget")} placeholder="e.g., $5,000, €3,000"/>
                  </div>
                </div>
              </div>)}
            ...
            <div className="grid gap-2">
              ...
              <Input id="city" {...register("city")} placeholder="e.g., New York, Paris, Tokyo"/>
            ...
            <div className="grid gap-2">
              ...
              <div className="flex w-full space-x-2">
                <Input id="hotel" {...register("hotel")} placeholder="e.g., Hotel Name, Airbnb address" onChange={(e) => setHotelSearchTerm(e.target.value)}/>
                <Button type="button" variant="outline" className="whitespace-nowrap px-3" onClick={async () => {
            ...
            try {
                ...
                if (aiData.locations && Array.isArray(aiData.locations)) {
                    ...
                    const processedLocations = await Promise.all(aiData.locations.map(async (loc: SharedLocType) => {
                        try {
                            const fullAddress = (loc.address || loc.name) + ", " +
                                (loc.city || city) + ", " +
                                (loc.region || "");
                            const mapboxResponse = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(fullAddress)}.json?access_token=${mapboxToken}&limit=1`);
                    ...
                    const validLocations = processedLocations.filter(loc => loc.latitude && loc.longitude);
              ...
              {hotelResults.length > 0 && (<div className="mt-2 max-h-40 overflow-y-auto border rounded-md">
                  {hotelResults.map((hotel, index) => (<Button key={index} type="button" variant="ghost" className="w-full p-2 h-auto text-left justify-start border-b last:border-b-0 rounded-none" onClick={() => {
                    ...
                    toast({
                      ...
                      {hotel.address && (<div className="text-xs text-muted-foreground">{hotel.address}</div>)}
                      {hotel.description && (<div className="text-xs text-muted-foreground mt-1">{hotel.description}</div>)}
                    </Button>))}
                </div>)}
        ...
        valueAsDate: true,
                ...
                {errors.startDate && (<p className="text-xs text-[hsl(var(--destructive))]">{errors.startDate.message}</p>)}
              </div>
        ...
        valueAsDate: true,
                ...
                {errors.endDate && (<p className="text-xs text-[hsl(var(--destructive))]">{errors.endDate.message}</p>)}
              </div>
            </div>
          </div>
