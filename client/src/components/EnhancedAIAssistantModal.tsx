...
export default function EnhancedAIAssistantModal({ isOpen, onClose, trip, activities, currentDate, onActivitiesUpdated }: EnhancedAIAssistantModalProps) {
        ...
        <Tabs defaultValue="chat" className="flex-1 flex flex-col min-h-0" value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="chat" className="flex-1 flex flex-col min-h-0 p-6 pt-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4 flex-shrink-0">
              <Button variant="outline" size="sm" className="text-left justify-start" onClick={() => setQuestion("I'd like to import my itinerary. I'll paste my schedule and you can add all activities to my trip.")}>
              ...
              <Button variant="outline" size="sm" className="text-left justify-start" onClick={() => setQuestion("Can you summarize my day and give me a brief overview?")}>
                <MessageSquare className="h-4 w-4 mr-2"/>
                <div>
            ...
            <ScrollArea className="flex-1 min-h-0 overflow-hidden pr-4">
              <div className="space-y-4">
                {conversation.map((message, index) => (<div key={index} className={`flex ${message.role === "assistant" ? "justify-start" : "justify-end"}`}>
                    <div className={`max-w-[80%] rounded-lg p-3 ${message.role === "assistant"
                ? "bg-muted text-foreground"
                : "bg-primary text-primary-foreground"}`}>
                      <div className="whitespace-pre-line">{message.content}</div>
                      {message.role === "assistant" && (message.content.toLowerCase().includes("food") || message.content.toLowerCase().includes("coffee") || message.content.toLowerCase().includes("restaurant") || message.content.toLowerCase().includes("cafe")) && (<div className="mt-3 space-y-2">
                          {/* Parse food suggestions and add buttons - multiple patterns */}
                          {(() => {
                    const suggestions = parseAISuggestions(message.content);
                    return suggestions.map((suggestion) => (<Button key={suggestion.key} variant="outline" size="sm" className="mr-2 mb-2" onClick={() => handleAddActivity({
            ...
            <div className="mt-4 flex items-end space-x-2 flex-shrink-0">
              <div className="flex-1">
                <Textarea placeholder="Ask me anything about your trip..." value={question} onChange={(e) => setQuestion(e.target.value)} className="min-h-[60px] max-h-[80px] resize-none" onKeyDown={(e) => {
            if (e.key === "Enter" && !e.shiftKey) {
              ...
              <Button onClick={handleSendQuestion} disabled={isProcessing || !question.trim()} className="h-10">
                Send
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="food" className="flex-1 flex flex-col min-h-0 p-6 pt-4">
            <div className="flex-1 min-h-0 overflow-auto">
              <FoodSuggestionsPanel trip={trip} onAddActivity={handleAddActivity}/>
            </div>
          </TabsContent>

          <TabsContent value="weather" className="flex-1 flex flex-col min-h-0 p-6 pt-4">
            <div className="flex-1 min-h-0 overflow-auto">
              <WeatherSuggestionsPanel trip={trip} onAddActivity={handleAddActivity}/>
            </div>
          </TabsContent>

          <TabsContent value="budget" className="flex-1 flex flex-col min-h-0 p-6 pt-4">
            <div className="flex-1 min-h-0 overflow-auto">
              <BudgetOptionsPanel trip={trip} onAddActivity={handleAddActivity}/>
            </div>
          </TabsContent>
        </Tabs>
      </DialogContent>
