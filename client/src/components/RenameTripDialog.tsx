...
export default function RenameTripDialog({ isOpen, onClose, trip }: RenameTripDialogProps) {
    ...
    const renameTrip = useMutation({
        mutationFn: async () => {
            if (!trip)
                throw new Error("No trip to rename");
            const response = await apiRequest("PUT", `${API_ENDPOINTS.TRIPS}/${trip.id}`, { title });
            if (!response.ok) {
    ...
    return (<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={onClose} disabled={renameTrip.isPending}>
              Cancel
            </Button>
            <Button type="submit" disabled={renameTrip.isPending || !title.trim()}>
