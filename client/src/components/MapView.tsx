  ...
  return (
    <section className="relative w-full h-full overflow-hidden map-container" style={{height: '100%', position: 'relative'}}>
      <div className="w-full h-full bg-[hsl(var(--muted))]" style={{height: '100%', position: 'relative'}}>
        ...
        {/* Map Footer Info */}
        {markers.length > 0 && (
          <div className="absolute bottom-4 left-4 bg-white dark:bg-[hsl(var(--card))] bg-opacity-95 dark:bg-opacity-95 px-3 py-2 rounded-lg shadow-lg">
            <h3 className="font-medium text-sm">Route Overview</h3>
            <p className="text-sm text-[hsl(var(--muted-foreground))]">
              {markers.length} stops
              {routes.length > 0 && routes[0]?.distance &&
                ` · ${(routes[0].distance / 1609.34).toFixed(1)} miles`}
            </p>
          </div>
        )}
