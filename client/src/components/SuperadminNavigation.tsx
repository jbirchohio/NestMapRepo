...
export function SuperadminNavigation() {
    ...
    return (<>
      ...
      <div className="lg:hidden fixed top-4 left-4 z-50">
        <Button onClick={() => setIsOpen(!isOpen)} variant="outline" size="sm" className="bg-navy-800/80 backdrop-blur-sm border-electric-700/30 text-white hover:bg-navy-700/80">
      ...
      {/* Sidebar */}
      <div className={cn("h-screen w-64 bg-navy-800 dark:bg-navy-900 border-r border-electric-200/30 dark:border-electric-700/30 flex flex-col transition-transform duration-300", "lg:translate-x-0 lg:fixed lg:left-0 lg:top-0", // Fixed positioning on desktop
        ...
        <nav className="flex-1 p-4 space-y-2">
          {superadminNavItems.map((item) => {
            ...
            return (<Link key={item.id} href={item.path}>
                <div onClick={() => setIsOpen(false)} // Close mobile menu on navigation
             className={cn('flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer morphing-button', isActive
        ...
        <div className="p-4 border-t border-electric-200/30 dark:border-electric-700/30 space-y-2">
          <Link href="/">
            <div onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-navy-300 hover:text-white hover:bg-navy-700/50 dark:hover:bg-navy-800/50 transition-all duration-200 cursor-pointer morphing-button">
              <BarChart3 className="w-5 h-5"/>
              Exit to App
            </div>
          </Link>
          <Button onClick={handleLogout} variant="ghost" className="w-full justify-start text-navy-300 hover:text-white hover:bg-navy-700/50 dark:hover:bg-navy-800/50">
