...
export function SystemStatusIndicator({ service, showIcon = false, className = "" }: SystemStatusProps) {
    ...
    return (<Badge variant={getBadgeVariant(status)} className={`${getBadgeColor(status)} ${className}`}>
      {showIcon && <Icon className="w-3 h-3 mr-1"/>}
      {text}
    </Badge>);
}
// Pre-configured components for common use cases
export const ApiHealthBadge = (props: Omit<SystemStatusProps, 'service'>) => <SystemStatusIndicator service="api" {...props}/>;
export const DatabaseStatusBadge = (props: Omit<SystemStatusProps, 'service'>) => <SystemStatusIndicator service="database" {...props}/>;
export const StripeStatusBadge = (props: Omit<SystemStatusProps, 'service'>) => <SystemStatusIndicator service="stripe" {...props}/>;
export const MonitoringStatusBadge = (props: Omit<SystemStatusProps, 'service'>) => <SystemStatusIndicator service="monitoring" {...props}/>;
export const PaymentModeBadge = (props: Omit<SystemStatusProps, 'service'>) => <SystemStatusIndicator service="payments" {...props}/>;
export const RegistrationStatusBadge = (props: Omit<SystemStatusProps, 'service'>) => <SystemStatusIndicator service="registration" {...props}/>;
