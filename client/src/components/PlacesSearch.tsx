...
export default function PlacesSearch({ onPlaceSelected, onClose, initialValue = "", placeholder = "Search for a place", className = "", }: PlacesSearchProps) {
    ...
    async function getCoordinates(address: string): Promise<[
        number,
        number
    ] | null> {
        try {
            const encodedAddress = encodeURIComponent(address);
            const response = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${encodedAddress}.json?access_token=${import.meta.env.VITE_MAPBOX_TOKEN}&limit=1`);
    ...
    return (<div className={`relative w-full ${className}`}>
      <div className="relative">
        <Input ref={inputRef} type="text" value={inputValue} onChange={handleInputChange} placeholder={placeholder} className="pr-10" onKeyDown={(e: React.KeyboardEvent) => {
            handleEscape(e);
            if (e.key === 'Enter') {
                e.preventDefault();
        ...
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
          {isLoading ? (<Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>) : (<Button type="button" onClick={() => inputValue.length >= 3 && refetch()} variant="ghost" size="icon" className="h-6 w-6 p-0">
      ...
      {showResults && (<div ref={resultsRef} className="absolute z-10 mt-1 w-full rounded-md border border-input bg-background shadow-md">
          {isLoading ? (<div className="flex items-center justify-center p-4">
              <Loader2 className="h-5 w-5 animate-spin text-muted-foreground mr-2"/>
              <span className="text-sm text-muted-foreground">
                Finding location...
              </span>
            </div>) : data ? (data.error ? (<div className="p-3 text-sm text-muted-foreground">
                Couldn't find that location. Try adding more details.
              </div>) : (<div className="p-3 flex items-start cursor-pointer hover:bg-muted" onClick={() => refetch()}>
                ...
                <div>
                  ...
                  {data?.description && (<div className="text-xs text-muted-foreground mt-1 line-clamp-2">
                      {data?.description}
                    </div>)}
                </div>
