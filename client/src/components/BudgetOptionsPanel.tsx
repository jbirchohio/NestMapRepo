    ...
    return (<div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Select value={budgetLevel} onValueChange={(value) => setBudgetLevel(value as BudgetLevel)}>
      ...
      <Button onClick={handleGetSuggestions} disabled={budgetMutation.isPending} className="w-full">
        {budgetMutation.isPending ? "Getting suggestions..." : "Get Budget Options"}
      </Button>
      
      {budgetMutation.isSuccess && budgetMutation.data && (<div className="mt-4 space-y-4">
          <div className="bg-muted p-4 rounded-md">
            ...
            <div className="mt-3">
              ...
              <ul className="text-sm mt-1 list-disc list-inside">
                {budgetMutation.data.budgetInfo.savingTips.map((tip, i) => (<li key={i}>{tip}</li>))}
              </ul>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {budgetMutation.data.suggestions.map((suggestion, index) => (<Card key={index} className="overflow-hidden">
            ...
            {budgetMutation.data.suggestions.length === 0 && (<p className="text-sm text-muted-foreground">No suggestions found for this budget level.</p>)}
          </div>
        </div>)}
    </div>);
}
