...
export default function CollaborationPresence({ 
  ...
  const getActivityIcon = (page: string) => {
    if (page.includes('activities')) return <Edit3 className="w-3 h-3" />;
    if (page.includes('map')) return <MapPin className="w-3 h-3" />;
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-3">
      ...
      {showActivityFeed && recentActivity.length > 0 && (
        <Card className="shadow-lg border-0 bg-white/95 backdrop-blur-sm max-w-80">
          <CardContent className="p-3">
      ...
      {/* Live Cursors */}
      {showCursors && collaborators.map((collaborator) => 
        collaborator.cursor && (
          <div
          ...
          >
            <div className="relative">
              ...
              <div 
                className="absolute top-5 left-2 text-xs font-medium text-white px-2 py-1 rounded-md shadow-sm whitespace-nowrap"
                style={{ backgroundColor: collaborator.color }}
              >
                {collaborator.username}
              </div>
