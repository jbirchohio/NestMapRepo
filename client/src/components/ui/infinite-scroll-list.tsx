 ...
 */
export function InfiniteScrollList<T>({ items, renderItem, loadMore, hasMore, isLoading, emptyMessage = 'No items found', className = '', loadingMessage = 'Loading more items...', }: InfiniteScrollListProps<T>) {
    ...
    if (items.length === 0 && !isLoading) {
        ...
        {typeof emptyMessage === 'string' ? (<p className="text-muted-foreground">{emptyMessage}</p>) : (emptyMessage)}
      </div>);
    }
    return (<div className={className}>
      ...
      {isLoading && (<div className="flex items-center justify-center py-4">
          {typeof loadingMessage === 'string' ? (<div className="flex items-center gap-2">
