...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      setToasts((toasts) =>
        toasts.filter((toast) => toast.open || Date.now() - toast.timestamp < TOAST_REMOVE_DELAY)
      )
    }, TOAST_REMOVE_DELAY)
...
const ToastProvider: React.FC<ToasterProps> = ({ children, ...props }) => {
  const [toasts, setToasts] = React.useState<React.ReactElement[]>([])

  const toastTimeouts: React.MutableRefObject<Map<string, ReturnType<typeof setTimeout>>> =
    React.useRef(new Map())

  const addToRemoveQueue = React.useCallback((toast: React.ReactElement) => {
    if (toast.key == null) {
      return
    }

    const timeout = setTimeout(() => {
      setToasts((t) => [...t.filter((t) => t.key !== toast.key)])
    }, TOAST_REMOVE_DELAY)

    toastTimeouts.current.set(toast.key, timeout)
  }, [])

  const addToast = React.useCallback((toast: React.ReactElement) => {
    setToasts((t) =>
      t.length >= TOAST_LIMIT ? [...t.slice(1, TOAST_LIMIT), toast] : [...t, toast]
    )

    addToRemoveQueue(toast)
  }, [addToRemoveQueue])

  const remove = React.useCallback((key: React.Key) => {
    setToasts((t) => [...t.filter((t) => t.key !== key)])
  }, [])
  ...
  const [toast] = React.useState(() => {
    ...
    return {
      ...
      toast({
        ...
        const key = `toast-${Math.floor(Math.random() * 1000000)}`
...
export { ToastProvider, ToastViewport, ToastClose, type UseToastReturn }
