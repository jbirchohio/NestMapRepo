...
const AnimatedCard = forwardRef<HTMLDivElement, AnimatedCardProps>(({ children, variant = "default", interactive = true, className, ...props }, ref) => {
    ...
    return (<motion.div ref={ref} variants={motionVariants} initial="initial" whileHover={interactive ? "hover" : undefined} whileTap={interactive ? "tap" : undefined} className={cn(baseStyles, variants[variant], "rounded-2xl p-6", interactive && "cursor-pointer", className)} {...props}>
        ...
        {variant === "glow" && (<div className="absolute -inset-1 bg-gradient-to-r from-electric-400/20 to-electric-600/20 rounded-2xl blur-sm opacity-75 animate-glow-pulse pointer-events-none"/>)}
      </motion.div>);
});
