...
export default function ShareTripModal({ isOpen, onClose, trip, onSave, }: ShareTripModalProps) {
    ...
    useEffect(() => {
        if (trip) {
            ...
            setSharePermission(trip.sharePermission || "read-only");
        ...
        <Tabs defaultValue="link">
          ...
          <TabsContent value="link" className="space-y-4">
            <div className="flex items-center justify-between space-x-2">
              ...
              <Switch id="public" checked={isPublic} onCheckedChange={handleTogglePublic}/>
            </div>
            
            <div className="flex items-center justify-between space-x-2">
              <Label htmlFor="sharing" className="flex-1">Enable sharing via link</Label>
              <Switch id="sharing" checked={sharingEnabled} onCheckedChange={handleToggleSharing}/>
            </div>
            
            {sharingEnabled && (<div className="space-y-4">
                <div className="space-y-2">
                  <Label>Link Permission</Label>
                  <div className="flex space-x-2">
                    <Button variant={sharePermission === "read-only" ? "default" : "outline"} size="sm" onClick={() => handlePermissionChange("read-only")} className="flex-1">
                      👀 View Only
                    </Button>
                    <Button variant={sharePermission === "edit" ? "default" : "outline"} size="sm" onClick={() => handlePermissionChange("edit")} className="flex-1">
                      ✏️ Can Edit
                    </Button>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="relative flex-1">
                      <Input id="shareLink" value={shareLink} readOnly disabled={!sharingEnabled}/>
                      {copied && (<CheckCircle className="absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-green-500"/>)}
                    </div>
                    <Button type="button" variant="outline" size="icon" onClick={copyToClipboard} disabled={!sharingEnabled}>
                ...
                <div className="pt-2">
                  <Button onClick={handleNativeShare} disabled={!shareLink} className="w-full" variant="default">
          ...
          <TabsContent value="collaborators" className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Input id="collaborator" placeholder="Email address" value={collaboratorEmail} onChange={(e) => setCollaboratorEmail(e.target.value)}/>
            ...
            <div className="space-y-2">
              ...
              {collaborators.length === 0 ? (<p className="text-sm text-muted-foreground">No collaborators added yet.</p>) : (<div className="space-y-2">
                  {collaborators.map((email) => (<div key={email} className="flex items-center justify-between bg-secondary p-2 rounded-md">
                      <div className="flex items-center space-x-2">
                        <Link className="h-4 w-4 text-muted-foreground"/>
                        <span>{email}</span>
                      </div>
                      <Button type="button" variant="ghost" size="sm" onClick={() => handleRemoveCollaborator(email)}>
                        Remove
                      </Button>
                    </div>))}
                </div>)}
