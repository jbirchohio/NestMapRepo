...
import { Palette, Eye, Sparkles, Building2, CheckCircle, ArrowRight, Crown } from 'lucide-react';
          ...
          <CardContent className="space-y-6">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <Switch checked={form.watch('enableBranding')} onCheckedChange={(checked) => form.setValue('enableBranding', checked)} disabled={!canUseBranding}/>
              </div>

              {/* Branding Configuration */}
              {form.watch('enableBranding') && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4">
                  <Separator />
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="companyName">Company Name</Label>
                      <Input id="companyName" {...form.register('companyName')} placeholder="Your Company Inc." className="bg-white dark:bg-navy-700"/>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Input id="primaryColor" type="color" {...form.register('primaryColor')} className="w-16 h-10 p-1 bg-white dark:bg-navy-700"/>
                        <Input value={form.watch('primaryColor')} onChange={(e) => form.setValue('primaryColor', e.target.value)} placeholder="#6D5DFB" className="flex-1 bg-white dark:bg-navy-700"/>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="tagline" {...form.register('tagline')} placeholder="Streamlined Business Travel" className="bg-white dark:bg-navy-700"/>
                  </div>

                  {/* Preview */}
                  {watchedValues.companyName && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 border rounded-lg bg-gradient-to-br from-white to-gray-50 dark:from-navy-800 dark:to-navy-700">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-6 rounded flex items-center justify-center text-white font-bold text-xs" style={{ backgroundColor: watchedValues.primaryColor }}>
                            {watchedValues.companyName.charAt(0)}
                          </div>
                          <span className="font-semibold" style={{ color: watchedValues.primaryColor }}>
                            {watchedValues.companyName}
                          </span>
                        </div>
                        {watchedValues.tagline && (<p className="text-sm text-muted-foreground">{watchedValues.tagline}</p>)}
                      </div>
                    </motion.div>)}
                </motion.div>)}
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button type="submit" disabled={isSubmitting || (!canUseBranding && form.watch('enableBranding'))} className="flex-1 bg-electric-600 hover:bg-electric-700">
                  {isSubmitting ? (<div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"/>) : (<>
            ...
            {/* Upgrade Prompt */}
            {!canUseBranding && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border border-orange-200 dark:border-orange-800 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
