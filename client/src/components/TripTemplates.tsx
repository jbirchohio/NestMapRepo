    ...
    return (<Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto" style={{
        ...
        {selectedTemplate ? (<div className="space-y-6">
            ...
            {selectedTemplate.budget && (<div className="flex items-center gap-2 text-sm">
                <DollarSign className="h-4 w-4"/>
                <span>Budget: ${selectedTemplate.budget.low} - ${selectedTemplate.budget.high} per day</span>
              </div>)}

            {selectedTemplate.bestTimeToVisit && (<div className="text-sm">
            ...
            <form onSubmit={handleCreateTrip} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="customTitle" value={customTitle} onChange={(e) => setCustomTitle(e.target.value)} placeholder={selectedTemplate.title}/>
                </div>
                <div>
                  <Label htmlFor="startDate">Start Date</Label>
                  <Input id="startDate" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} min={today} required/>
                </div>
              </div>

              {startDate && (<div className="text-sm text-muted-foreground">
                  Trip dates: {formatDate(startDate)} - {getEndDate(startDate, selectedTemplate.duration)}
                </div>)}

              <div className="flex gap-2">
                <Button type="submit" disabled={!startDate || createTripMutation.isPending} className="flex-1">
            ...
            {isLoading ? (<div className="text-center py-8">Loading templates...</div>) : (<div className="grid gap-4">
                {templates?.map((template: TripTemplate) => (<div key={template.id} className="border rounded-lg p-4 hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => setSelectedTemplate(template)}>
                    <div className="flex items-start justify-between">
                      ...
                      {template.budget && (<div className="text-right text-sm">
