...
export default function SwipeableTrip({ trip, onNavigate, onRename, isGuestMode }: SwipeableTripProps) {
    ...
    const handleMouseDown = () => {
        if (swiped)
            return; // Don't trigger long press when swiped
    ...
    return (<div className="relative overflow-hidden rounded-md" {...swipeHandlers}>
      {/* Delete button revealed on swipe */}
      <div className={`absolute inset-y-0 right-0 flex items-center transition-transform duration-200 ease-out ${swiped ? "transform translate-x-0" : "transform translate-x-full"}`} style={{ width: "70px" }}>
        <Button variant="destructive" size="icon" className="h-full w-full rounded-none" onClick={handleDelete} disabled={isDeleting}>
          {isDeleting ? (<div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent"/>) : (<Trash2 className="h-5 w-5"/>)}
        </Button>
      </div>

      {/* Card with transform based on swipe state */}
      <div className={`transition-transform duration-200 ease-out ${swiped ? "transform -translate-x-[70px]" : "transform translate-x-0"}`}>
        <Card className={`cursor-pointer hover:shadow-md transition-shadow ${longPress ? "bg-[hsl(var(--muted))]" : ""}`} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} onTouchStart={handleMouseDown} onTouchEnd={handleMouseUp} onTouchMove={handleMouseMove}>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">
                    Client: {(trip as any /** FIXANYERROR: Replace 'any' */).clientName || trip.city || "Internal Project"}
                  </span>
