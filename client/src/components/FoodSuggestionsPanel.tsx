...
export default function FoodSuggestionsPanel({ trip, onAddActivity }: FoodSuggestionsPanelProps) {
    ...
    return (<div className="space-y-4">
      <div className="text-center">
        ...
        <Button onClick={handleGetSuggestions} disabled={foodMutation.isPending} className="w-full">
      ...
      {suggestions.length > 0 && (<div className="space-y-3">
          ...
          {suggestions.map((suggestion, index) => (<Card key={index} className="border">
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center">
                  {suggestion.type.toLowerCase().includes('coffee') ? (<Coffee className="h-4 w-4 mr-2 text-amber-600"/>) : (<UtensilsCrossed className="h-4 w-4 mr-2 text-orange-600"/>)}
                  {suggestion.name}
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                ...
                <Button onClick={() => handleAddFood(suggestion)} variant="outline" size="sm" className="w-full">
                  Add to Trip
                </Button>
              </CardContent>
            </Card>))}
