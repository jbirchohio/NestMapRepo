...
export function CardsList({ cards, onCardAction, isLoading }: CardsListProps) {
    ...
    if (isLoading) {
        ...
        {[...Array(3)].map((_, index) => (<Card key={index} className="animate-pulse">
    ...
    return (<div className="space-y-4">
      {cards.length === 0 ? (<Card>
        ...
        </Card>) : (cards.map((card) => (<Card key={card.id} className="hover:shadow-md transition-shadow">
            <CardContent className="p-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {card.status === 'active' ? (<DropdownMenuItem onClick={() => onCardAction(card.id, 'freeze')}>
                          <Lock className="h-4 w-4 mr-2"/>
                          Freeze Card
                        </DropdownMenuItem>) : card.status === 'frozen' ? (<DropdownMenuItem onClick={() => onCardAction(card.id, 'unfreeze')}>
                          <Unlock className="h-4 w-4 mr-2"/>
                          Unfreeze Card
                        </DropdownMenuItem>) : null}
                      <DropdownMenuItem onClick={() => onCardAction(card.id, 'suspend')} className="text-red-600 dark:text-red-400">
              ...
              <div className="mb-4">
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${getSpendingPercentage(card.monthlySpent, card.spendingLimit)}%` }}/>
                </div>
              </div>

              <div className="flex justify-between items-center text-sm text-gray-600 dark:text-gray-400">
                <div className="flex gap-4">
                  {card.department && (<span>Department: {card.department}</span>)}
                  {card.purpose && (<span>Purpose: {card.purpose}</span>)}
                </div>
