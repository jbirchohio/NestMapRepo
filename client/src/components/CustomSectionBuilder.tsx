      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select onValueChange={(value: SectionType) => addSection(value)}>
            ...
            <SelectContent>
              {sectionTypes.map(type => (<SelectItem key={type.value} value={type.value}>
                  <div>
                      ...
                      {(provided: DraggableProvided, snapshot: DraggableStateSnapshot) => (<Card ref={provided.innerRef} {...provided.draggableProps} className={`${snapshot.isDragging ? 'shadow-lg' : ''} ${!section.enabled ? 'opacity-50' : ''}`}>
                          <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                <Switch checked={section.enabled} onCheckedChange={(enabled) => updateSection(section.id, { enabled })}/>
                                <Button variant="outline" size="sm" onClick={() => setEditingSection(editingSection === section.id ? null : section.id)}>
                                  <Edit3 className="w-4 h-4"/>
                                </Button>
                                <Button variant="outline" size="sm" onClick={() => deleteSection(section.id)} className="text-red-600 hover:text-red-700">
                          ...
                          <CardContent>
                            {editingSection === section.id ? (<div className="space-y-4">
                                <div>
                                  <Label>Section Title</Label>
                                  <Input value={section.title} onChange={(e) => updateSection(section.id, { title: e.target.value })} placeholder="Enter section title"/>
                                </div>
                                <div>
                                  <Label>Content</Label>
                                  <Textarea value={section.content} onChange={(e) => updateSection(section.id, { content: e.target.value })} placeholder={section.type === 'list' ? 'Enter items separated by new lines (use • for bullets)' :
                                section.type === 'table' ? 'Use | to separate columns and --- for headers' :
                                    'Enter your content here...'} rows={6}/>
                                </div>
                              ...
                              </div>) : previewMode ? (<SectionPreview section={section}/>) : (<div className="text-sm text-gray-600 dark:text-gray-300">
...
function SectionPreview({ section }: {
    ...
    const renderContent = () => {
        switch (section.type) {
            ...
            {section.content.split('\n').map((paragraph, index) => (<p key={index} className="mb-2 last:mb-0">
                {paragraph}
              </p>))}
          </div>);
        }
    };
    return (<div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-800/50">
