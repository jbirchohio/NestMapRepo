...
export default function ItinerarySidebar({ trip, activities, todos, notes, activeDay, onChangeDayClick, onActivitiesUpdated, onAddActivity, mobileView, setMobileView, }: ItinerarySidebarProps) {
    ...
    return (<>
        ...
        <Tabs defaultValue="itinerary">
          ...
          <TabsContent value="itinerary" className="space-y-4">
            ...
            <div className="mb-4">
              <div className="grid grid-cols-2 gap-2">
                {trip.days?.map((day, index) => (<Button key={day.toISOString()} variant={day.toDateString() === activeDay.toDateString() ? "default" : "outline"} className="px-3 py-2 text-sm md:text-base h-auto" onClick={() => onChangeDayClick(day)}>
            ...
            if (onAddActivity) {
              ...
              {/* AI Assistant Button */}
              <Button variant="outline" className="w-full" onClick={() => setIsAIModalOpen(true)}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="secondary" size="sm" className="flex-1" onClick={handleAutoOptimize} disabled={activities.length === 0 || isAutoOptimizing}>
                  {isAutoOptimizing ? (<>
                ...
                <Button variant="outline" size="sm" className="flex-1" onClick={() => setIsOptimizationModalOpen(true)} disabled={activities.length === 0}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M16 3h5v5"/>
                    <path d="m21 3-5 5"/>
                    <path d="M8 3H3v5"/>
          ...
          <TabsContent value="todo">
            <div className="space-y-4">
              <form onSubmit={handleSubmitTodo} className="flex space-x-2">
                <input type="text" placeholder="Add a new task..." className="flex-1 px-3 py-2 border dark:border-[hsl(var(--border))] rounded-md focus:outline-none focus:ring-2 focus:ring-[hsl(var(--ring))]" value={newTodoText} onChange={(e) => setNewTodoText(e.target.value)}/>
              ...
              <div className="space-y-2">
                {todos.length === 0 ? (<p className="text-center py-4 text-[hsl(var(--muted-foreground))]">No tasks yet. Add one above!</p>) : (todos.map((todo) => (<div key={todo.id} className="flex items-center space-x-2 p-2 hover:bg-[hsl(var(--muted))] rounded-md">
                      <Checkbox id={`todo-${todo.id}`} checked={todo.completed ?? false} onCheckedChange={() => toggleTodo.mutate(todo)}/>
                      <label htmlFor={`todo-${todo.id}`} className={`flex-1 cursor-pointer ${todo.completed ? 'line-through text-[hsl(var(--muted-foreground))]' : ''}`}>
                        {todo.task}
                      </label>
                    </div>)))}
              </div>
          ...
          <TabsContent value="notes">
            <form onSubmit={handleSubmitNotes}>
              <textarea className="w-full h-64 p-3 border dark:border-[hsl(var(--border))] rounded-md focus:outline-none focus:ring-2 focus:ring-[hsl(var(--ring))]" placeholder="Add notes for your trip..." value={newNote || notes} onChange={(e) => setNewNote(e.target.value)}></textarea>
      ...
      {/* AI Assistant Modal */}
      <EnhancedAIAssistantModal isOpen={isAIModalOpen} onClose={() => setIsAIModalOpen(false)} trip={trip} activities={activities} currentDate={activeDay} onActivitiesUpdated={() => {
            queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.TRIPS, trip.id, "activities"] });
        }}/>

      {/* AI Itinerary Optimization Modal */}
      <ItineraryOptimizationModal open={isOptimizationModalOpen} onOpenChange={setIsOptimizationModalOpen} trip={trip} activities={activities} onApplyOptimization={(optimizedActivities) => {
