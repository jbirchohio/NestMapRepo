...
import { Calendar, CalendarDays, CheckCircle, ExternalLink, Download, Upload, Settings, AlertCircle, Info } from 'lucide-react';
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => exportCalendarMutation.mutate('current')} disabled={exportCalendarMutation.isPending}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {isLoading ? (<div className="flex items-center justify-center py-8">
            ...
            </div>) : connections.length === 0 ? (<Alert>
              ...
              {connections.map((connection: CalendarConnection) => (<div key={connection.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{getProviderIcon(connection.provider)}</span>
                    <div>
                      <div className="font-medium flex items-center gap-2">
                        {getProviderName(connection.provider)}
                        {connection.connected && (<Badge variant="secondary" className="text-green-600">
                      ...
                      <div className="text-sm text-slate-600 dark:text-slate-400">
                        {connection.email}
                        {connection.lastSync && (<span className="ml-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Switch checked={connection.syncEnabled} onCheckedChange={(checked) => {
                    // Update sync setting
                    console.log(`Toggle sync for ${connection.id}: ${checked}`);
                }}/>
                    </div>
                    
                    <Button variant="outline" size="sm" onClick={() => syncTripsToCalendarMutation.mutate(connection.id)} disabled={syncTripsToCalendarMutation.isPending}>
                      Sync Now
                    </Button>
                    
                    <Button variant="ghost" size="sm" onClick={() => disconnectCalendarMutation.mutate(connection.id)} className="text-red-600 hover:text-red-700">
                      Disconnect
                    </Button>
                  </div>
                </div>))}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => connectCalendarMutation.mutate('google')} disabled={connectCalendarMutation.isPending} className="flex items-center gap-2">
                🗓️ Google Calendar
              </Button>
              
              <Button variant="outline" onClick={() => connectCalendarMutation.mutate('outlook')} disabled={connectCalendarMutation.isPending} className="flex items-center gap-2">
                📅 Outlook
              </Button>
              
              <Button variant="outline" onClick={() => connectCalendarMutation.mutate('apple')} disabled={connectCalendarMutation.isPending} className="flex items-center gap-2">
                🍎 Apple Calendar
              </Button>
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Switch checked={settings.autoSync} onCheckedChange={(checked) => setSettings(prev => ({ ...prev, autoSync: checked }))}/>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked={settings.includeNotes} onCheckedChange={(checked) => setSettings(prev => ({ ...prev, includeNotes: checked }))}/>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked={settings.setReminders} onCheckedChange={(checked) => setSettings(prev => ({ ...prev, setReminders: checked }))}/>
          </div>

          <div className="space-y-2">
            <div className="font-medium">Sync Frequency</div>
            <select className="w-full p-2 border rounded-md" value={settings.syncInterval} onChange={(e) => setSettings(prev => ({ ...prev, syncInterval: parseInt(e.target.value) }))}>
