      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleOptimizeTrips} disabled={isOptimizing} className="bg-blue-600 hover:bg-blue-700">
              {isOptimizing ? (<>
          ...
          <Card>
            ...
            <CardContent>
              <ul className="space-y-2">
                {optimizationResult.recommendations.map((rec, index) => (<li key={index} className="flex items-start gap-2">
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {optimizationResult.optimizedTrips.map((trip) => (<TableRow key={trip.id}>
                        <TableCell>
                          <div>
                        ...
                        <TableCell>
                          {trip.hasOptimization ? (<div>
                              ...
                              {trip.reasoning && (<p className="text-xs text-muted-foreground mt-1">
                                  {trip.reasoning}
                                </p>)}
                            </div>) : (<span className="text-sm text-muted-foreground">No changes needed</span>)}
                        </TableCell>
                        <TableCell>
                          <div>
                            <p className="text-sm">
                            ...
                            {trip.savings > 0 && (<p className="text-sm text-green-600 font-medium">
                                Save ${trip.savings}
                              </p>)}
                          </div>
                        </TableCell>
                        <TableCell>
                          {trip.conflictFlags.length > 0 ? (<Badge variant="destructive" className="text-xs">
                        ...
                        <TableCell>
                          {trip.hasOptimization && !appliedChanges.has(trip.id) ? (<div className="flex gap-1">
                              <Button size="sm" variant="outline" onClick={() => handleApplyChanges(trip.id)} className="text-green-600 border-green-600 hover:bg-green-50">
                                <Check className="h-3 w-3"/>
                              </Button>
                              <Button size="sm" variant="outline" onClick={() => handleRejectChanges(trip.id)} className="text-red-600 border-red-600 hover:bg-red-50">
                                <X className="h-3 w-3"/>
                              </Button>
                            </div>) : appliedChanges.has(trip.id) ? (<Badge variant="outline" className="text-green-600">
                              Applied
