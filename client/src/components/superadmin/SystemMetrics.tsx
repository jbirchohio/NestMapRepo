...
export function SystemMetrics({ dashboardData, isLoading }: SystemMetricsProps) {
    if (isLoading) {
        ...
        {[...Array(4)].map((_, index) => (<Card key={index} className="animate-pulse">
    ...
    const calculateSystemHealth = () => {
        ...
        if (failedJobs > 0 && failedJobs / totalJobs > 0.1)
            return { status: 'critical', percentage: 60 };
        if (totalSessions > 100)
            return { status: 'warning', percentage: 80 };
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <div className={`h-12 w-12 rounded-lg flex items-center justify-center ${systemHealth.status === 'healthy'
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              {dashboardData.backgroundJobs && dashboardData.backgroundJobs.length > 0 ? (<div className="space-y-2">
                ...
                return (<div key={status} className="space-y-1">
                        ...
                        <Progress value={percentage} className={`h-2 ${status === 'failed' ? 'bg-red-100' :
                        status === 'completed' ? 'bg-green-100' : 'bg-gray-100'}`}/>
                      </div>);
            })}
                </div>) : (<p className="text-sm text-gray-500 dark:text-gray-400">No background jobs</p>)}
            </div>
          </CardContent>
        </Card>
