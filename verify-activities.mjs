...
async function checkActivities() {
  ...
  }).from(templates).where(inArray(templates.id, templateIds));
  console.log("\n=== TEMPLATE ACTIVITIES CHECK ===\n");
  for (const template of results) {
    const activities2 = template.trip_data?.activities || [];
    console.log(`Template ${template.id}: ${template.title}`);
    console.log(`  Activities: ${activities2.length}`);
    if (activities2.length > 0) {
      console.log(`  First activity: ${activities2[0].title}`);
      console.log(`  Has coordinates: ${!!activities2[0].latitude}`);
    }
    console.log("");
  }
  process.exit(0);
