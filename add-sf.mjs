...
var sfActivities = {
  ...
  activities: [
    ...
    { day: 3, title: "Castro District", time: "5:00 PM", locationName: "Castro Street", latitude: 37.7609, longitude: -122.435, price: 0, costNotes: "FREE walking", notes: "Historic LGBTQ neighborhood" },
    // Day 4
...
async function addSFActivities() {
  ...
  const [template] = await db.select().from(templates).where(like(templates.title, "%SF Broke%")).limit(1);
  if (!template) {
