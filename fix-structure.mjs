...
async function fixActivitiesStructure() {
  ...
  const results = await db.select().from(templates).where(inArray(templates.id, templateIds));
  for (const template of results) {
    ...
    console.log(`  \u2713 Fixed structure for ${days.length} days with ${activities2.length} total activities`);
  }
